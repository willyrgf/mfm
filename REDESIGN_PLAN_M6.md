...
- [x] **Fact durability at IO-call boundary**
  ...
  - Implemented in `crates/machine/src/live_io.rs` + `crates/machine/src/runtime/attempt.rs`.
- [x] **`RunConfig.skip_tags`**
  ...
  - Implemented in `crates/machine/src/runtime/attempt.rs` with a regression test in `crates/machine/src/runtime.rs`.
...
- [x] **Enforce side-effect idempotency contracts**
  ...
  - Enforcement: **planner validation** in `crates/sdk/src/unstable.rs` (`DefaultPipelinePlanner`) (fail fast during plan construction).
