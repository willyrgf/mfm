...
> Version: v3 (draft contract updates referenced by `REDESIGN_PLAN_V3.md`)
...
- A state attempt is delimited by kernel events and MAY span multiple appends (§6.2/§6.4).
...
- Milestone 1 note: engine-level child-run spawning/linkage is explicitly deferred; Milestone 1 assumes flattened
  composition only (§11).
...
**Constraints**
- No NaN/Infinity.
- Hashed structures MUST NOT contain floats (prefer integer-scaled or decimal strings).
...
- Secrets (never store in context, events, or artifacts; secret-bearing artifacts are deferred for Milestone 1).
...
### 6.2 Transactionality (Per-Append) + Attempt Envelopes
...
- `StateEntered { state_id, attempt, base_snapshot_id }` (attempt start)
...
A single attempt MAY span multiple appends. This is required to avoid “IO happened but wasn’t recorded” windows for
fact recording.
...
- Domain events emitted during an in-flight attempt remain in the append-only stream. On resume:
  ...
  - other domain events MUST NOT advance plan progression or snapshot selection (audit-only)
...
- Kernel event semantics:
  ...
  - The terminal kernel event is the commit marker for checkpoint advancement:
    - only `StateCompleted { context_snapshot_id }` advances the run’s effective checkpoint
    - `StateFailed` MUST NOT advance the checkpoint (`failure_snapshot_id` is diagnostic-only)
...
* A `FactKey` is **single-assignment**: the first durable `FactRecorded { key, payload_id }` binds the key permanently
  to that payload. This enables deterministic retries and crash/resume for read-only IO.
...
Rules:
- In Live mode, replayable IO SHOULD provide `fact_key` and produce fact payload artifacts.
  If a call has no `fact_key`, it is treated as **non-replayable** by default.
- In Replay mode, any IO call that influences deterministic behavior MUST provide `fact_key`.
  If `fact_key` is absent in Replay mode, return a structured `IoError` with stable code `missing_fact_key` and
...
- nested machines (deferred for Milestone 1; flattened composition only)
...
Milestone 1 note: parallel execution and engine-managed child runs are deferred. Treat this as a later-milestone
composition pattern; Milestone 1 uses flattened, sequential composition.

1. `FanOut` state:

   * enumerates work items
   * (later milestone) spawns child runs per item and records child references
...
Milestone 1 note: nested-machine execution (engine-level child-run spawning/linkage) is explicitly deferred; Milestone 1
assumes flattened composition only.
...
### 11.2 Nested machines spawned inside a state (deferred for Milestone 1)
...
* Milestone 1 rule: **no secrets in artifacts**, including fact payloads and context snapshots.
* Encrypted secret-bearing artifacts (if ever added) are explicitly deferred until after Milestone 1.
...
* if needed, store references:
  ...
  * keystore entry IDs
  * key/provider reference IDs (never secret contents)
...
pub mod ids {
    ...
    /// Example: "portfolio_tracker" or "portfolio_management.prices".
...
pub mod io {
    ...
    pub struct IoCall {
        ...
        /// Optional fact key for recording/replay. Required for replayable IO in Replay mode (§8).
...
pub mod recorder {
    ...
    ///   kernel event).
    /// - A state attempt MAY span multiple transactional event-store appends; each append is atomic.
    #[async_trait]
