...
Status: implemented. [docs/design.md](docs/design.md) owns the current contract.
[Validation evidence](docs/recovery-validation.md) records the completed replacement, enrichment,
and composed CI results.

The [API sketch and engineer handoff](docs/rfc-recovery-api-sketch.md) records the design intent and
links to production consuming examples for typed composition, checked values, checkpoint scoping,
and persistence.
...
These sketches specify the mapping order. The [API appendix](docs/rfc-recovery-api-sketch.md) links
to production consuming tests for finite binding selection, checked values, and Runtime association.
...
1. **Record the agreed proposal.** Documentation only; specify public composable classification and
   recovery, the linked type/signature appendix, derived data, fixed stop
   ...
   arithmetic through production consuming tests.
...
none. The implementation resolves the former validation questions:

- Production Portfolio policy tests cover finite typed bindings, original failure preservation,
  inherited recovery, and occurrence overrides; Runtime tests cover exact association and reports.
- Scoped authoring compile-fail tests and Runtime nested restart tests cover checkpoint authority,
  restored inputs, and independent global/local allowance exhaustion.
- EVM adapter and transaction tests cover State context, anchor confirmation, cancellation, and
  pending Effect authority without changing the prepare/conclusion lifecycle.
- Portfolio enrichment and application tests cover bounded output, exact immutable revision
  publication, repeated publication without provider IO, and dependent admission.
- Maximum Portfolio and transaction fixtures establish useful complete-history bounds.

Concrete measurements and the completed 12-task composed CI run are recorded in
[recovery validation](docs/recovery-validation.md). Inline reports intentionally retain both original
and mapped failures; admission does not guarantee that their combined report fits the 32 MiB limit.
The documented size-limit error preserves the acknowledged prefix and pending Effect authority.
