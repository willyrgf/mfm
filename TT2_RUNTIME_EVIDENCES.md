...
- The managed qualification now injects four child-process exits (status 137)
  ...
  proves an existing-completion idempotent replay transaction whose
...
- The shared configuration append boundary now rejects an oversized serialized
  ...
  and one-byte-over `MAX_STORED_FRAME_BYTES` unit witnesses for both envelope
  and retained object frames. One-level-over depth, float, duplicate-key, and
  malformed values are rejected before backend dispatch; the complete
  generated,
...
| `bc469cc8` | exercise evm crash recovery boundary |
...
| `1b984001` | bind postgres fault gates to executed connections |
| `e02895ad` | yield between configuration checkpoint reads |
...
| Canonical batch/count boundary | `49d2e77c` | `219c588b` adds exact 65,536-record/object and empty/one-over witnesses; `49d2e77c` applies the same predicate to nested arrays/objects at exact 1,048,576 and one over; focused canonical-append target passes 5/5 | PASS for these exact/one-over count boundaries; broader batch/corpus matrix remains conditional |
...
Revision `78dbc354` extends that focused target through the retained-object
validator with a valid exact `MAX_STORED_FRAME_BYTES` object. Revision
`e5f8c3b0` then routes both envelope and object checks through one shared
byte-length predicate (minimum 2 for envelopes, minimum 1 for objects), so the
...
Revision `219c588b` routes record and object cardinality through one shared
predicate: records accept counts in `[1, 65,536]`, objects accept `[0, 65,536]`,
...
Revision `49d2e77c` reuses that predicate for nested JSON arrays and objects;
both accept `[0, 1,048,576]` and reject one over before recursive traversal.
The same 5/5 canonical-append target and independent review mark the nested
...
Revision `70e6900e` routes both prior-run source-manifest history-object
encoding and decoding through one private byte validator. The generated
16,777,216-byte manifest budget is accepted exactly and rejected at one byte
...
Revision `17129b21` makes the native-wire canonical-value validator enforce the
generated string, object-key, array-item, and object-entry budgets before
recursive traversal. The recoverability generator now names the 1,048,576-byte
object-key budget and derives all native shape limits from the generated source;
direct validator witnesses accept each exact limit and reject one over. The
focused `mfm-canonical` target passes 1/1, with the full canonical integration
targets passing 3/11/5; independent review marks this native-boundary proof
PASS.

Revision `26bbb44d` applies the generated 22,369,622-character
`MAX_BASE64URL_CHARACTERS` budget at the untrusted `CanonicalBytes` parser before
character scanning or decoded allocation. The exact budget decodes to 16,777,216
bytes and one character over is rejected; the focused canonical-json target
passes 1/1 and independent review marks the parser boundary PASS.

Revision `6f135845` removes the unreachable signed variant from the native
primitive recoverability schema and regenerates the annex, corpus, README
metadata, and derived schema identities. The primitive codec rejects signed
JSON numbers, and `FactScalar` no longer exposes a misleading signed constructor
or accepts signed producer values; direct canonical and fact regressions pass.
Full canonical targets pass 3/12/6 plus 3 doctests, the facts target passes
...
Revision `60554999` extends that independent negative corpus with operation-key
...
committed. The fresh resume worker then closes the same run and the test
asserts exactly one `eth_sendRawTransaction` call, preserving the existing
provider/application and no-secret-output checks:
...
Revision `a11925b7` adds a fourth managed child exit after the application has
returned its closed response (and the completion is durable). A final normal
resume reloads that completed run, preserves the persisted closure, and still
asserts exactly one raw broadcast:

```text
nix run .#run -- --task evm-postgres-submission-qualification
run id: run-2422160-1786021998684634852
result: ok — 1 passed, 0 failed in 1620.25s
```

This is a post-closure process-loss witness. The managed wallet authority
separately qualifies an existing-completion idempotent replay whose
acknowledgement is lost: exact-key retry returns the original closure and the
persisted completion count remains one (`run-2436682-1786023975122635293`, 1/1
...
revision `9f61c39a`; later focused ingress-boundary revisions are
`e5f8c3b0`, `219c588b`, and `49d2e77c`):
...
`6cd74eef`; `bc469cc8` adds one post-broadcast crash/restart witness, while
production deployment/provider trust and the broader crash, ambiguity,
latency, and production-authority matrices remain conditional;
