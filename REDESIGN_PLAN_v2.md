# MFM — REDESIGN.md Implementation Plan (v2.1)
...
>
> Version: v2.1 (integrates `REDESIGN_CRITIC_v2.md`)
...
# Core Semantic Clarifications (v2)
...
**Note:** `REDESIGN.md` currently states stricter “atomic state transition emission” semantics (§6.4). This plan’s
model requires a contract update; track this in §Deviations and resolve before implementation starts.
...
* D-01 State transition atomicity: `REDESIGN.md` §6.4 currently requires “atomic state transition emission,” but S1
  allows a single attempt to span multiple transactional appends (needed for crash-safe fact recording). Resolve by
  updating `REDESIGN.md` to match S1 before implementation begins.
...
**Intent:** Make the plan auditable by removing the biggest plan↔contract contradiction before code lands.

### Steps

1. Update `REDESIGN.md` §6.4 to match S1:
...
2. Ensure any “atomic transition emission” language is updated or constrained to match the chosen model.

### Acceptance

* D-01 is removed (or updated with new references) because plan and contract agree.
...
* [ ] D-01 deviation resolved (contract updated, or plan adjusted) before implementation proceeds
