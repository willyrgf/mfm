...
The historical `evm_contract_effect_recovers_cold_and_accepts_external_nonce_advance` failure
occurred after its third pending operational failure exceeded the former fixture's two-record
quota. Its helper did not expose the operational causes, so those logs cannot explain the failures.

The auditability capacity cutover removes that quota repository-wide. Semantic recovery limits and
actual Store/frame limits remain. This contract change does not identify or fix the earlier
provider failure; the two historical runs remain failed evidence, and the later passing run alone
