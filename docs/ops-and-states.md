...
| `evm_deploy_configure_validate` | `v1` | `crates/ops/evm-deploy-configure-validate-op` | Compatibility root that accepts canonical-or-built deploy/configure/validate config; canonical input composes `evm_deploy_configure_validate_config_build` before `evm_deploy_configure_validate_execute`, while built input goes straight to execution | Child build workflow plus `evm_deploy`, `evm_configure`, `evm_validate` | `mfm run pipeline deploy-configure-validate`, feature `pipeline.deploy_configure_validate.start`, feature `run.start` |
...
| `portfolio_tracker` | `v1` | `crates/ops/portfolio-tracker-op` | Compatibility root that accepts canonical-or-built portfolio config; canonical input composes `portfolio_config_build` before the fixed semantic runtime, while built input goes straight to the runtime | Child build workflow plus `PrepareExecutionSourcesState`, `ResolveSubjectsState`, `PinExecutionViewsState`, `ResolveValuationInputsState`, `ObserveCompiledBatchState`, `MergeObservationsState`, `AssembleSnapshotState`, `ProjectReportState` | `mfm portfolio snapshot`, feature `portfolio.snapshot`, feature `run.start` |
...
| `aave_v3_origin_stack` | `v1` | `crates/ops/aave-v3-origin-op` | Compatibility root that accepts canonical-or-built Aave V3 Origin config; canonical input composes `aave_v3_origin_stack_config_build` before `aave_v3_origin_stack_execute`, while built input goes straight to execution | Child build workflow plus `nix_app`, `aave_v3_origin_adapt_deploy`, internal publication/report helper ops | feature `run.start` |
