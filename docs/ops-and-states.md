...
| `crates/states/portfolio/src/semantic_states.rs` | `PrepareExecutionSourcesState`, `ResolveSubjectsState`, `PinExecutionViewsState`, `ResolveValuationInputsState`, `ObserveCompiledBatchState`, `MergeObservationsState`, `AssembleSnapshotState`, `ProjectReportState` | Fixed semantic runtime states for source preparation, subject/view resolution, valuation resolution, batch observation, and snapshot/report projection | `portfolio_tracker` |
| `crates/states/wallet/src/states.rs` | `ResolveWalletsState` | Legacy reusable wallet-resolution flow kept for compatibility tests and non-semantic slices | No direct built-in use today |
| `crates/states/symbol/src/states.rs` | `ReadDirectPricesState`, `CollectObservationsState`, `MergeObservationsState` | Legacy reusable valuation-source execution and observation merge slices kept compatible with the semantic pinned-view contract | No direct built-in use today |
| `crates/states/portfolio/src/states.rs` | `PinPortfolioNetworksState`, `WritePortfolioSnapshotState`, `WritePortfolioReportState` | Legacy reusable network pinning and snapshot/report projection states kept for compatibility tests and transition coverage | No direct built-in use today |
| `crates/states/aave-v3/src/portfolio/states.rs` | `CollectAaveObservationsState` | Legacy Aave V3 observation collector retained as a reusable compatibility slice while semantic adapters own the built-in runtime path | No direct built-in use today |
