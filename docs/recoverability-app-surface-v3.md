...
PendingEffect {
    ...
    executor_status:
        ...
        "did_not_enter" |
        "indeterminate" |
        "non_domain_retryable" |
        "non_domain_integrity_blocked",
}
...
closed evidence. The wrapper is not persisted, receives no synthetic `ValueRef`, and grants no
object authority. A non-domain observation can never be the consumed observation of a valid
settlement.
...
non-domain failure, pending ensure result, wrong request transition, or mismatched authorization,
effect, or request identity fails integrity verification. Its evidence contains each verified external-observation
...
AccessAuditEntry {
    ...
    non_domain_failure: NonDomainFailure | null,
...
`non_domain_failure` is non-null exactly when status is `non_domain_failure`; `failure`,
`non_domain_failure`, and `result_ref` are otherwise mutually constrained by the selected status.
The non-domain value is audit-only and carries no diagnostic reference, provider text, or open
extension. Its code fixes the permitted access layer, conservative entry status, and disposition.
It is never passed to state logic or rendered as a domain failure. `retryable_operational` permits
only a later separately authorized access after this record commits; `integrity_blocked` blocks
automatic progress.
...
- audit serialization admits the closed `NonDomainFailure` code/status/disposition relation only
  in `non_domain_failure`, while replay and settlement tests prove it is callback-free,
  state-nonconsumable, and never rendered as a safe or domain failure;
- every surviving live return is committed or byte-identically resolved before a successful drive
  result; stale-head and acknowledgement-ambiguous observation retries preserve exact pending
  bytes, never reinvoke, and task cancellation starts no detached completion;
