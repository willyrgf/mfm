...
| Store metadata | `store_metadata` | No secrets. Fields are internal non-secret metadata. `trust_scope_id` is non-secret deployment identity material. | operational telemetry | Defines store epoch, store-owned trust scope, and schema contract version. Runtime credentials must not mutate this table. |
