...
| EVM runtime source provenance | Typed EVM evidence fields in run events, facts, and artifacts | No secrets allowed. May include redacted `selected_source_ref`, `policy_id`, observed `chain_id`, semantic `network_id`, and semantic expected chain id. Must not include RPC URLs, auth headers, file paths, signer material, or signed raw transactions. | audit provenance | `selected_source_ref` is the actual source that served the call after policy fallback. These fields are for auditability only; replay must not resolve them against current runtime config, and retry, consistency, public-output, and side-effect authority must not depend on them. |
