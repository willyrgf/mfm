...
| Fact descriptor projection | `fact_descriptor_index` | No secrets allowed. Contains descriptor hashes, schema ids, fact kind, compatibility group, subject namespace hash, artifact ids/evidence hashes, and source run coordinates. | rebuildable cache | Searchable descriptor catalog derived from descriptor artifacts and run authority. It is not semantic authority; rebuild and replay verify against retained descriptor artifacts. |
