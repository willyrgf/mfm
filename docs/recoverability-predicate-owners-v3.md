...
| `P-HB-05` | Hostile provider or executor response bytes become the exact reviewed typed result, the shared `mfm-capabilities::SafeFailure` selected by `safe_failure_contract_ref`, or a closed audit-only `NonDomainFailure` when the surviving boundary return or returned-value totalization is operationally unavailable or violates its contract. Unrepresentable bytes are neither retained nor fingerprinted. The safe-failure universal class includes cooperative `cancellation`; conservative custody maps safe outcomes to exact `DidNotEnter`/`Indeterminate` and non-domain outcomes to exact entry status, disposition, and code. | The admitted capability or executor classifier/canonicalizer executed inside the audited wrapper. | Only the private runtime/executor wrapper can consume affine authority and construct sealed `PendingObservation<K>` or use the executor's private completion seal to bind an unbound target outcome. Raw bytes, open diagnostic maps, callback-authored receipt metadata, and outer post-invocation errors have no persistence path. | Callback-free observation append/load and replay verify the complete four-outcome algebra, safe-failure contract relation, non-domain layer/code/status/disposition relation, bounds, wrapper binding, and semantic non-consumability. Classifier-bound reads additionally strict-decode the classifier and verify its exact embedded diagnostic `SchemaIdentity`, derived schema id, and complete structural `SchemaShape`; reference-executor failures instead verify their exact contract-specific tuple. Neither path reparses discarded bytes, invents a better classification, or passes non-domain evidence to a state callback. |
...
The non-domain vocabulary is exactly `adapter_contract_violation | result_encoding_failure |
fact_store_unavailable | fact_history_invalid | executor_store_unavailable |
executor_contention | executor_history_invalid | executor_capacity_exhausted`. Its entry status is
`proven_not_entered | may_have_entered`; its disposition is `retryable_operational |
integrity_blocked`. The frozen schema owns the complete code/status/disposition and access-layer
relation. Non-domain outcomes contain no diagnostic and are never state-consumable.
...
| `P-ST-03` | Per-run compare-and-swap, append-request idempotency/conflict, logical admission uniqueness, record/slot uniqueness, one closure, and one observation per authorization are enforced atomically. | The store backend behind `RunHistoryWriter`. | Stable run-root/admission-key locking, exact predecessor reload, unique keys, and one transaction or infallible memory swap. | Load/replay verify immutable uniqueness and predecessor linkage. Runtime retries but cannot declare a stale append successful. |
| `P-ST-04` | Store-assigned run sequence, record ordinal/id, tagged tenant fact coordinate, routing copies, candidate/record/commit digests, and final `JournalHead` are derived from the validated batch under the exact canonical preimages. | The store backend behind `RunHistoryWriter`. | Store-only coordinate assignment occurs after validation and before all-or-nothing publication. | Shared codec, memory/PostgreSQL parity, replay, and trace/export rederive hashes and routing. Callers never author assigned coordinates. |
| `P-ST-05` | Object-path bindings and artifact-admission intents are complete, exact, sorted, mode-consistent, and atomic with the first authoritative record reference; preexisting input cannot self-admit. An attested fact observation alone may carry first-use cross-run authorities as unbound `RequireExisting` imports. | The run-history writer object-admission contract. | `RequireExisting` versus `AdmitOrVerifyExact` is derived from closed field paths and committed with the batch. Physical replay defers only the attested, unbound fact-import case; the semantic observation fold must exact-close the complete imported graph before constructing a view, while the live backend still proves global authority. | Both backends and replay compare bound identity sets, evidence, bytes, modes, producer role, and no extras/omissions. Tests reject an ordinary observation's first-use `RequireExisting` authority and extra or substituted fact imports. |
| `P-ST-06` | A live access permit exists only for a directly observed winning `NewlyAppended` authorization; `ExistingSame`, reload, stale loss, commit ambiguity, or reconciliation mints none. | The runtime-owned writer authorization-result constructor. | Non-cloneable `NewlyAppendedAuthorization` is returned only on the positive commit path and consumed by runtime. | Runtime checks its exact binding before constructing affine access. Tests reconcile ambiguous appends without authority. Persisted records alone cannot recreate the permit. |
| `P-ST-07` | One logical observation is the exact authorization reference plus complete persistable content and remains stable across predecessor changes. It has one committed matching authorization, exact wrapper/binding/request/schema, one of four closed outcomes, at most one value, byte-identical resolution, conflict on changed content, and legal pre-closure or constrained late-tail placement. A predecessor-bound physical append id remains unchanged while acknowledgement is ambiguous and is replaced only after a definite stale-head result. | `Runtime::commit_observation<K>` owns the pending obligation and sequencing; the run-history writer/store owns logical-key resolution, candidate legality, and atomic append. | Private `PendingObservation<K>` is retained independently of every physical candidate. Only a positive commit or exact byte-identical logical-key resolution constructs private `CommittedObservation<K>`; neither path can reinvoke. | Structural reload/replay verifies authorization chain, role, all four outcome schemas, non-domain non-consumability, uniqueness, exact content, and late-tail legality. Retry tests inject stale predecessors, acknowledgement ambiguity, missing/conflicting observations, capped backoff, and task cancellation. The verifier cannot synthesize an observation for an unmatched authorization. |
| `P-ST-08` | The canonical node/run fold, state digest, binding delta, effect slot, deterministic skip reason, terminality, closure digest, and post-closure audit-only behavior agree for the whole batch. | The shared structural verifier executed behind writer and reader purpose surfaces. | Store applies the complete candidate fold or nothing; human-readable phases/deltas are checked redundancy only. | Runtime derives candidates from the same fold; replay reconstructs every historical prefix. Projection/checkpoint data cannot override it. |
| `P-ST-09` | A backend publishes no head, idempotency entry, object authority, coordinate, or partial row set before every fallible validation succeeds. | Each `RunJournalBackend` implementation under the common atomicity contract. | Memory performs one infallible staged swap; PostgreSQL performs one transaction and commits last. | Injected-failure parity compares memory and PostgreSQL for every legal batch and coordinate. |

## Audited capabilities and recoverable executors

| Id | Predicate | Sole owner | Seal or enforcement point | Required redundant verification |
| --- | --- | --- | --- | --- |
| `P-EX-01` | One private `Prepared<K>` completes request-side binding, type, codec, contract, route, and encoder selection before authorization. Only a directly acknowledged authorization mints affine `Authorized<K>`, which permits zero or one independently meaningful application-protocol operation with no hidden retry, redirect, failover, provider reselection, adaptive subcall, or independent batch member. Every surviving return totalizes without an outer error into one non-escaping `PendingObservation<K>`, and normal success requires `CommittedObservation<K>`. | Runtime's single sealed access bracket using the exact qualified capability/executor wrapper. | Private `PreparedAccess`, `Authorized<K>`, `PendingObservation<K>`, and `CommittedObservation<K>` constructors enforce the stage order; local rejection yields `DidNotEnter`, while operational/contract/encoding faults use the closed non-domain relation. | Compile-fail, capability, runtime retry, and adversarial transport tests verify one-entry behavior, no pending escape, exact-content persistence, and no reinvocation. Journal audit proves prior authorization and a surviving committed outcome, not exact packet or remote-call count. |
...
| `P-EX-05` | Executor delivery evidence is one bounded immutable prefix chain from `EffectBound` through allocation, target authorizations/observations, and optional terminal tombstone; same/equal/ancestor/descendant handling, finite bounds, and fork rejection follow the exact executor contract. `execute_target_once` alone commits authorization, invokes one affine target closure, validates and binds its unbound outcome through the retained private completion seal, and persists or exactly resolves that attempt's observation before returning. A schema-valid result over the retained bound is explicit `ResultUnrepresentable`. | The qualified executor ledger and its admitted pure frontier verifier. | Positively acknowledged executor-internal append mints target-entry authority. The completion seal never escapes the method, and the callback supplies no identity, attempt, operation, or generation metadata; observation ambiguity, conflict, tombstone, and restart resolve retained history and cannot call the target a second time. | Generic memory/file/PostgreSQL conformance brackets the target, injects crash/acknowledgement boundaries, checkpoints and restores, proves one target entry and byte-stable terminal evidence, and rejects a second execution. MFM runtime/store/replay verify predecessor, proof, bounds, identity, tombstone, and all target outcomes without querying mutable executor state. |
...
For `P-EX-04`–`P-EX-06`, EVM target preflight is exactly the private five-variant
`PreparedEvmWalletTarget` sum: `Broadcast`, `TransactionLookup`, `ReceiptLookup`,
`FinalizedHead`, and `CanonicalInclusion`. Each variant makes exactly one transport call.
Post-exchange classification has three disjoint paths: valid encoded bytes above the retained
bound become `ResultUnrepresentable`; invalid typed/contract results become
`adapter_contract_violation/MayHaveEntered/IntegrityBlocked`; encoding or schema construction
failure becomes `result_encoding_failure/MayHaveEntered/IntegrityBlocked`.
...
| `P-FA-02` | The reserved scan returns exactly the qualifying facts from other runs in the admitted tenant through its authorization frontier, with verified producer transitions/objects/content identities and deterministic predicate, ordering, limit, and tie-break. | The non-overridable `mfm.journal.fact-selection.v1` capability over Runtime's same writer backend. | One winning affine authorization creates a private, non-cloneable, non-serializable `FactScanSession` containing the authorization, request, exact frontier, next `(fact_order, fact_ordinal)`, and bounded top-K accumulator. `FactScanPageVerifier` alone applies the 4,096-publication/8,192-emission step budgets and logical ranges after a backend submits each ordered complete publication. Each contiguous writer step consumes the session and returns only private `More`; reaching the frontier constructs affine `CompletedFactScan`, which is consumed exactly once into immutable sealed `PendingObservation<FactSelection>` material before any physical append candidate. Step budgets never truncate or invalidate the total prefix, and app cannot inject another fact store. | Recorded-history verification uses the same scanner from sealed replay authority to detect selected-item alteration or omission; exact reproduction may compare the request. Exact-scale core tests cross each budget within a publication, while genuine memory and PostgreSQL workflows verify restart, fixed-barrier exclusion, nonempty attestation, semantic reload, omission rejection, stale-head/ambiguous observation retry without rescanning, and returned store errors as closed fact-layer non-domain outcomes. Any future candidate index must rehydrate from this authority and cannot decide completeness. |
| `P-FA-03` | A fact-emitting transition gets exactly one positive dense tenant publication order; a selection authorization snapshots the current tenant head without advancing it; rollback creates no gap; same-tenant publication/barrier commits are totally ordered and different tenants are independent. | The writer backend's tenant fact-coordinate allocator. | The fact publication or barrier is assigned under one tenant-head lock held through commit and covered by `commit_digest`. | Store open verifies head/publication/barrier union, density, range, tenant and routing copies. Memory/PostgreSQL parity rechecks structure; no caller or replica assigns a coordinate. |
...
| `P-PG-07` | Every live authority-bearing read and all writes use the fenced authoritative writer; a replica, apparent catch-up, or connection option cannot construct current store-backed authority. | Deployment/store connection authority qualified by `P-PG-06`. | App/store assembly exposes only the writer pool for admission, journal/object/fact reads, status, replay, trace/audit/export, resume, and writes. | Tests reject replica-configured authority. Offline verification remains `P-ACL-04`; a future replica contract would need exact lineage plus applied barrier/publication proof. |
...
- Production
  ...
  full-publication validation around
  ...
  authorization keeps its fixed barrier while a later matching publication is excluded, and a
  normal observation persists one nonempty attestation. Reload restores semantic output/fact
...
- The current [runtime access protocol](../crates/kernel/runtime/src/access_protocol.rs) and
  ...
  cancellation with no detached completion for `P-EX-01` and `P-ST-07`.
...
- The current [EVM wallet target tests](../crates/live/evm/src/wallet_executor_tests.rs) exercise all
  five prepared target variants through the same one-call boundary, byte-stable restart/tombstone
  recovery, and the disjoint oversized-valid, invalid-typed, and canonical-encoding-failure
  classifiers for `P-HB-05` and `P-EX-04`–`P-EX-06`.
