...
pub struct EntryKdfParams { // Renamed from MasterKdfParams
...
impl Keystore {
    ...
    pub fn initialize_or_load(&mut self) -> Result<(), KeystoreError> { // Removed password parameter
    ...
    pub fn import_private_key_hex(
        ...
        if let Some(ref new_alias) = alias {
            if self.entries.iter().any(|e| e.alias.as_ref() == Some(new_alias)) {
    ...
    pub fn change_password(
        ...
        let entry_index = self.entries.iter().position(|e| e.id == uuid)
            .ok_or(KeystoreError::KeyNotFound(uuid))?;
        ...
        let nonce_vec = BASE64_STANDARD
            ...
            .map_err(|_| KeystoreError::InvalidFormat)?;
        let old_nonce_bytes: [u8; 12] = nonce_vec.try_into().map_err(|_| KeystoreError::InvalidFormat)?;
    ...
    fn encrypt_pk(
        ...
        id: &Uuid,
        address: &Address,
    ) -> Result<(Vec<u8>, [u8; 12]), KeystoreError> { // Return includes nonce
    ...
    fn derive_key_for_entry( // Renamed from derive_master_key
        &self,
        password: &str,
    ...
    fn generate_entry_kdf_params( // Name is already generate_entry_kdf_params
        rng: &mut (impl CryptoRng + RngCore),
        config: &KeystoreConfig,
    ) -> Result<EntryKdfParams, KeystoreError> { // Return type is correct
        // Fix for F-6: Enforce minimum output length of 32 bytes
        if config.output_len < 32 {
            return Err(KeystoreError::FsError(
                "KDF output length must be at least 32 bytes".to_string(),
        ...
        Ok(EntryKdfParams { // Return EntryKdfParams
