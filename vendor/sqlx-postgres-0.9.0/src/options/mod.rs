...
impl PgConnectOptions {
    /// Creates deterministic compiled-default options without reading environment variables,
    /// user identity, home-directory state, passfiles, service files, or the filesystem.
    ///
    /// Every authority-bearing field must be set explicitly by the caller before connecting.
    pub fn new_without_env_or_files() -> Self {
        PgConnectOptions {
    ...
    /// Sets the exact immutable Rustls root store for this connection.
    ///
    /// Unlike `ssl_root_cert_from_pem`, this replaces every compiled or ambient root source.
    pub fn ssl_root_cert_store(mut self, roots: std::sync::Arc<rustls::RootCertStore>) -> Self {
        self.ssl_root_cert = Some(CertificateInput::ExclusiveRootStore(roots));
        self
    }
