...
pub enum CertificateInput {
    ...
    File(PathBuf),
    /// One exact caller-supplied immutable Rustls root store.
    #[cfg(feature = "_tls-rustls")]
    ExclusiveRootStore(std::sync::Arc<rustls::RootCertStore>),
}
...
impl CertificateInput {
    async fn data(&self) -> Result<Vec<u8>, std::io::Error> {
        ...
        match self {
            ...
            #[cfg(feature = "_tls-rustls")]
            CertificateInput::ExclusiveRootStore(_) => Err(std::io::Error::new(
                std::io::ErrorKind::InvalidInput,
                "an exact root store is not certificate byte input",
            )),
        }
    }
}
...
impl std::fmt::Display for CertificateInput {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            #[cfg(feature = "_tls-rustls")]
            CertificateInput::ExclusiveRootStore(_) => f.write_str("exact Rustls root store"),
        }
    }
}
