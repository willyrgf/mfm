...
async fn setup_cli_import_list_and_export_preserve_the_catalog_contract() {
    ...
    let identity = values
        .iter()
        .find(|value| value["name"] == "acme/bitcoin-balance")
        .expect("BTC identity");
    ...
    let first_output = directory.path().join("btc-one.json");
    let second_output = directory.path().join("btc-two.json");
    let exported = json_output(run_cli(
        &scoped_url,
        &export_args("acme/bitcoin-balance", schema_id, digest, &first_output),
    ));
    assert_eq!(
    ...
    json_output(run_cli(
        &scoped_url,
        &export_args("acme/bitcoin-balance", schema_id, digest, &second_output),
    ));
