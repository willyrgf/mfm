...
`Setup` is a sized checked, bounded, deterministic, secret-free product. It may contain public
content/binding references but never providers, clients, signers, private keys, credentials,
nonce-authority handles, Store connections, Runtime handles, or mutable ambient state. A domain
normally owns the product. The six current EVM identity policies deliberately use the exact kernel
`ContentRef` that the trusted Portfolio planner derives once from a checked target and shares with
C0 and the configured child Operation. `original_binding_ref` derives or clones the binding from
the same setup used by the hooks. It neither registers nor selects a live adapter.
...
original_binding_ref(setup) = setup.clone()
write_before = empty
write_after = empty
```
...
Program bytes as today. The trusted Portfolio planner derives each checked target binding once and
shares the same value between the child's C0 demand and `CollectEvmBalances`; supplying a binding
does not confer provider authority, because Runtime assembly still requires the exact trusted
composition registration before provider entry.
...
- the exact checked target `ContentRef`; and
- source count in `1..=EVM_BALANCE_SOURCE_LIMIT`.

The trusted Portfolio planner derives the binding once from `EvmPhysicalTarget`, shares it with the
child's C0 demand, and moves it into the child. The owned binding keeps the public Operation type
lifetime-free without retaining the otherwise unused target. A private non-owning marker represents
`K` when no other field does.
...
for _ in 0..self.source_count {
    ...
    body.match_join::<EvmBalanceAsset<K>, EvmBalanceContext<K>>(|arms| {
        arms.arm::<EvmBalanceContext<K>>(native.clone(), |native| {
            native.read::<ReadNativeBalance<K>, EvmBalanceRead>(&self.binding_ref)
        })?;

        arms.arm::<EvmBalanceContext<K>>(token.clone(), |token| {
            token.read::<ReadTokenDecimals<K>, EvmBalanceRead>(&self.binding_ref)?;
            token.read::<ReadTokenBalance<K>, EvmBalanceRead>(&self.binding_ref)
        })
    })?;
...
This is explanatory source shape. Production uses checked StableId construction and exact tags.
Match is necessary because the selected value changes future Read topology: neither branch
executes, calls, or journals the other branch's capability path at Runtime.
...
Current Portfolio planning rejects more than 64 target descriptors before cloning inputs or
entering Runtime. It clones only those bounded, secret-free inputs into an immediately awaited
blocking task for pure synchronous planning; Runtime progression remains asynchronous.
...
### 10.4 Live EVM and App

Change trusted registration and composition tests to semantic types. Live EVM retains only adapter
registration and no Portfolio dependency. Application's production API remains unchanged; Tokio is
a normal dependency solely for the bounded, immediately awaited blocking planning boundary.
