...
preparation are owned by `mfm-evm-dcv-model`, backed by `mfm-evm-core::abi`. Keep ABI semantics
there so planner validation and runtime execution share one contract without making config crates
depend on runtime.
