...
use mfm_evm_contract_config::{
    ...
    EvmNetworkIntent, ValidatePhaseConfig,
};
use mfm_evm_contract_model::{
    ConfiguredContract, ConfiguredContractInstance, ContextBoundValidationReport, DeployedContract,
    DeployedContractInstance, EvmContractContext, ValidationReport,
};
...
use mfm_state_evm_contracts::{
    ...
    ContextBoundConfigureContractState, ContextBoundDeployContractState,
    ContextBoundValidateContractState, ContextConfigureContractInputHandles,
    ContextValidateContractInputHandles, ContractLifecyclePublicOutputs, DeployContractState,
    ImportConfiguredContractState, ImportDeployedContractState, ValidateContractInputHandles,
...
pub fn context_deploy_contract_program_draft(
...
pub fn context_configure_contract_program_draft(
...
pub fn context_validate_contract_program_draft(
...
pub fn context_contract_lifecycle_program_draft(
...
/// Plans a context-bound deploy-only EVM contract program.
pub fn plan_context_contract_deploy_entry_point(
    config: EvmContractDeployEntryConfig,
) -> Result<TypedProgramLaunchPlan, ContractLifecyclePlanError> {
    Ok(TypedProgramLaunchPlan::from_draft(
        context_deploy_contract_program_draft(config)?,
    )?)
}

/// Plans a context-bound configure-only EVM contract program through an import node.
pub fn plan_context_contract_configure_entry_point(
    config: EvmContractConfigureEntryConfig,
) -> Result<TypedProgramLaunchPlan, ContractLifecyclePlanError> {
    Ok(TypedProgramLaunchPlan::from_draft(
        context_configure_contract_program_draft(config)?,
    )?)
}

/// Plans a context-bound validate-only EVM contract program through an import node.
pub fn plan_context_contract_validate_entry_point(
    config: EvmContractValidateEntryConfig,
) -> Result<TypedProgramLaunchPlan, ContractLifecyclePlanError> {
    Ok(TypedProgramLaunchPlan::from_draft(
        context_validate_contract_program_draft(config)?,
    )?)
}

/// Plans a context-bound full lifecycle EVM contract program.
pub fn plan_context_contract_lifecycle_entry_point(
    config: EvmContractLifecycleEntryConfig,
) -> Result<TypedProgramLaunchPlan, ContractLifecyclePlanError> {
    Ok(TypedProgramLaunchPlan::from_draft(
        context_contract_lifecycle_program_draft(config)?,
    )?)
}
