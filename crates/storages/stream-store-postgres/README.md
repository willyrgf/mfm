...
(`certified_spec_hash` plus `trust_scope_id`) and store the concrete holder run
...
`store_metadata.trust_scope_id` is generated by the baseline migration,
...
library maintenance endpoint for store-epoch reseed, cursor pruning, trust-scope
