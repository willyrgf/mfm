...
ownership. Execution-claim rows are nowait holder leases keyed by base work
identity (`certified_spec_hash` plus `trust_scope_id`) with a separate holder
run id; they coordinate active drivers and require explicit holder-and-token
matched reaping after expiry before another holder can acquire. `store_metadata.trust_scope_id` is
