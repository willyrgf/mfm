...
//!
//! [`PostgresTypedRunEventStore`] is the certified typed run-event store for typed-core execution.
//! The legacy [`PostgresStreamStore`] persists pre-typed dynamic streams in separate `mfm_*`
//! tables and is not a certified typed run submit/resume surface.
//!
...
impl PostgresStreamStore {
    ...
    /// Returns the certified typed run event store sharing this PostgreSQL connection.
    ///
    /// The legacy `StreamStore` implementation remains available for existing dynamic callers, but
    /// certified typed runs must use this typed store surface.
    pub fn typed_run_event_store(&self) -> PostgresTypedRunEventStore {
        PostgresTypedRunEventStore::from_client(self.client.clone())
    }
