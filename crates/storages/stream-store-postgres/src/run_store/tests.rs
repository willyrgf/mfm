...
async fn store_authority_rejects_schema_drift_cases() {
    ...
    for (case, expected) in [
        ...
        (
            Case::InvalidTrustScopeBinding,
            PostgresStoreAuthorityError::TrustScope,
        ),
        (
        ...
        match case {
            ...
            Case::InvalidTrustScopeBinding => {
                mutate_store_metadata_unchecked(
                    &store.pool,
                    &[
                        "ALTER TABLE store_metadata ALTER COLUMN trust_scope_id DROP NOT NULL",
                        "UPDATE store_metadata SET trust_scope_id = NULL",
                    ],
                )
                .await;
            }
...
fn fact_authority_spec() -> spec::TypedExecutionSpec {
...
fn run_identity_material_for_fact_run_id(run_id: &RunId) -> events::RunIdentityMaterialV1 {
    (0..=u8::MAX)
