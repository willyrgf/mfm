...
pub(super) async fn load_store_metadata(pool: &PgPool) -> Result<StoreMetadata> {
    let row = sqlx::query("SELECT store_epoch, store_scope_id FROM store_metadata WHERE singleton")
        .fetch_one(pool)
        .await
    ...
    Ok(StoreMetadata {
        ...
        store_scope_id: StoreScopeId::new(
            row.try_get::<String, _>("store_scope_id")
                .map_err(|error| database_error("failed to decode store scope id", error))?,
        )?,
    })
}

pub(super) async fn load_store_scope_id_client(pool: &PgPool) -> Result<StoreScopeId> {
