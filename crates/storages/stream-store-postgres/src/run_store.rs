...
fn final_commit_authority_from_parts(
    run_id: &RunId,
    seq: StreamSeq,
    commit_key: &CommitKey,
    commit_id: &str,
...
fn sorted_artifact_binding_authority_json(
    binding_kind: &str,
    evidence: &[ArtifactEvidenceRef],
) -> Result<Vec<Value>> {
...
async fn validate_final_commit_authority_tx(
    tx: &mut Transaction<'_, Postgres>,
...
struct CommitArtifactBindings {
    required: Vec<ArtifactEvidenceRef>,
    admitted: Vec<ArtifactEvidenceRef>,
}

async fn load_commit_artifact_bindings_tx(
    tx: &mut Transaction<'_, Postgres>,
    commit: &CommitAuthorityRow,
...
mod tests {
    ...
    fn expected_resource_lane_id_v1(evidence: &events::ResourceKeyEvidence) -> Result<[u8; 32]> {
        ...
        let mut input =
            Vec::with_capacity(RESOURCE_LANE_ID_DOMAIN.len() + canonical.as_bytes().len());
        input.extend_from_slice(RESOURCE_LANE_ID_DOMAIN);
        input.extend_from_slice(canonical.as_bytes());
        let digest = sha256_digest_bytes(&input);
        let mut lane_id = [0_u8; 32];
