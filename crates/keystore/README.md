...
and returns key- and immutable-purpose-bound `KeystoreSigner` handles. Purpose stays on the handle
and never crosses the owner channel. Duplicate same-key imports converge on the same retained key
instance even when their handles have different purposes; at most 64 distinct keys are retained.
