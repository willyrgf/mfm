...
use mfm_evm::{
    EvmAddress, EvmBalanceRead, EvmBalanceTarget, EvmBlockAnchor, EvmHash, EvmReadEvidence,
...
fn target(address_byte: &str) -> EvmBalanceTarget {
...
fn intent(target: EvmBalanceTarget, anchor: EvmBlockAnchor) -> EvmReadIntent {
    EvmReadIntent::new(
        0,
        mfm_chain::balance::DecimalScale::new(6).unwrap(),
...
fn broad_evm_read_contract_ids_and_wires_are_exact() {
    ...
    assert_eq!(
        ...
        [
            (
                "semantic:mfm.derived:evm_read_subject:2:sha256-jcs-v1:1de056303272f6c89c8d2a6e4cca19bb4f09f48756c4fd561a19efcbafbc9656".to_owned(),
                "schema:mfm.derived.evm_read_subject:2:sha256-jcs-v1:ed9b7183b04241d3a74b5b212bf83873feb958945784b4da3c22273b916010bd".to_owned(),
                "{\"token_balance\":{\"anchor\":{\"hash\":\"0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb\",\"number\":\"17\"}}}".to_owned(),
            ),
            (
                "semantic:mfm.derived:evm_read_intent:2:sha256-jcs-v1:844d97eb17ae22b4ad6fe75bb2311e8d65ca5cf3479b0fb97761aa4706a1cf7d".to_owned(),
                "schema:mfm.derived.evm_read_intent:2:sha256-jcs-v1:e3d5976b296177d711fcb66bcd761cb2b84929f934d07428bd3aed66fa6728df".to_owned(),
                "{\"chain_id\":1,\"collection_scale\":6,\"route_ref\":{\"content_digest\":\"content:sha256-v1:0202020202020202020202020202020202020202020202020202020202020202\",\"schema_id\":\"schema:mfm.test.route:1:sha256-jcs-v1:0101010101010101010101010101010101010101010101010101010101010101\"},\"source_ordinal\":0,\"subject\":{\"token_balance\":{\"anchor\":{\"hash\":\"0xbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb\",\"number\":\"17\"}}},\"target\":{\"account\":\"0xaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa\",\"token\":\"0xcccccccccccccccccccccccccccccccccccccccc\"}}".to_owned(),
            ),
            (
            ...
            (
                ...
                "{\"kind\":\"returned\",\"value\":{\"intent_value_ref\":{\"content_digest\":\"content:sha256-v1:9a61309bc59f23f65dbb90bfe82040fd13490abaf3193c465fa05e47a0ecb8e6\",\"schema_id\":\"schema:mfm.derived.evm_read_intent:2:sha256-jcs-v1:e3d5976b296177d711fcb66bcd761cb2b84929f934d07428bd3aed66fa6728df\"},\"value\":{\"kind\":\"token_decimals\",\"value\":18}}}".to_owned(),
            ),
        ]
    );
    assert_eq!(
...
fn broad_evidence_rejects_cross_intent_substitution() {
    let original = intent(target("aa"), anchor(17, "bb"));
    ...
    let (_, native_ref) = canonicalize_mfm_value(&evidence).unwrap();
    EvmBalanceRead::bind_evidence(&original_ref, &original, &native_ref, &evidence)
        .expect("bound evidence");

    for substituted in [
        intent(target("dd"), anchor(17, "bb")),
        intent(target("aa"), anchor(18, "ee")),
    ] {
        let (_, substituted_ref) = canonicalize_mfm_value(&substituted).expect("substituted ref");
        assert!(EvmBalanceRead::bind_evidence(
            &substituted_ref,
            &substituted,
            &native_ref,
            &evidence
...
fn every_broad_terminal_evidence_wire_carries_the_exact_intent_ref() {
    let intent = intent(target("aa"), anchor(17, "bb"));
    let (_, intent_value_ref) = canonicalize_mfm_value(&intent).expect("intent ref");
...
fn broad_read_decode_rejects_unchecked_or_incomplete_values() {
    ...
    subject_wire["token_balance"]["extra"] = serde_json::json!(true);
