...
use mfm_state_portfolio::{
    assemble_snapshot, btc_holding_candidate, evm_holding_candidate, expand_required_holdings,
    holding_fact_index_request, observations_from_selected_holdings, portfolio_adapter_kind,
    portfolio_adapter_version, project_network_pins_from_observations, resolve_subjects_from_config,
...
async fn select_holdings(
    ...
    for requirement in &requirements {
        let request = holding_fact_index_request(config, requirement).map_err(holding_runtime_error)?;
        ...
        match requirement.projection {
            HoldingFactProjection::BitcoinAddressBalance => {
                ...
                match btc_holding_candidate(requirement, &hydrated, store_commit_order, claim_id) {
                    ...
                    {
                        // Drop non-acceptable candidates; pure select fails if set empty.
                    }
