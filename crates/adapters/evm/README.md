...
exact-anchor validation. The balance executor resolves one anchor, performs bounded ordered native
and ERC-20 reads, and retains complete reducer evidence. The record runner atomically publishes the
complete `evm.balance_snapshot` batch with its checked receipt.
