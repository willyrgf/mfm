...
#[test]
fn coverage_and_holding_source_status_tags_and_admission_rules() {
    assert_eq!(
        CoverageStatus::CompleteAtAnchor.as_str(),
        "complete_at_anchor"
    );
    assert_eq!(CoverageStatus::ConfiguredOnly.as_str(), "configured_only");
    assert_eq!(CoverageStatus::Truncated.as_str(), "truncated");
    assert_eq!(CoverageStatus::Incomplete.as_str(), "incomplete");
    assert_eq!(
        "complete_at_anchor"
            .parse::<CoverageStatus>()
            .expect("parse"),
        CoverageStatus::CompleteAtAnchor
    );
    assert!("not_a_coverage".parse::<CoverageStatus>().is_err());

    assert!(CoverageStatus::CompleteAtAnchor.is_admissible_for_write());
    assert!(CoverageStatus::ConfiguredOnly.is_admissible_for_write());
    assert!(!CoverageStatus::Truncated.is_admissible_for_write());
    assert!(!CoverageStatus::Incomplete.is_admissible_for_write());
    assert!(CoverageStatus::ConfiguredOnly.is_acceptable_for_report());
    assert!(!CoverageStatus::Truncated.is_acceptable_for_report());

    assert_eq!(HoldingSourceStatus::Ok.as_str(), "ok");
    assert_eq!(HoldingSourceStatus::Unsupported.as_str(), "unsupported");
    assert_eq!(HoldingSourceStatus::Failed.as_str(), "failed");
    assert_eq!(
        "ok".parse::<HoldingSourceStatus>().expect("parse"),
        HoldingSourceStatus::Ok
    );
    assert!("synced".parse::<HoldingSourceStatus>().is_err());

    assert!(HoldingSourceStatus::Ok.is_admissible_for_write());
    assert!(!HoldingSourceStatus::Unsupported.is_admissible_for_write());
    assert!(!HoldingSourceStatus::Failed.is_admissible_for_write());
    assert!(HoldingSourceStatus::Ok.is_acceptable_for_report());
    assert!(!HoldingSourceStatus::Failed.is_acceptable_for_report());
}
