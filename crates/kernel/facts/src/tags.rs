...
/// Coverage claim carried on holding and similar source-near data facts.
///
/// Writers may only admit [`Self::CompleteAtAnchor`] or [`Self::ConfiguredOnly`].
/// [`Self::Truncated`] and [`Self::Incomplete`] fail closed before Platform write.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
pub enum CoverageStatus {
    ...
    Truncated,
    /// Observation is incomplete and must not be written as a Platform holding fact.
    Incomplete,
}

impl_fact_tag!(CoverageStatus, "coverage status", pub, "Returns the canonical snake-case tag used on holding fact responses.", {
    Self::CompleteAtAnchor => "complete_at_anchor",
...
impl CoverageStatus {
    ...
    /// Returns whether a report candidate with this coverage is acceptable in v1.
    pub const fn is_acceptable_for_report(self) -> bool {
        self.is_admissible_for_write()
    }
}

/// Source status for holding (and similar) data facts — not Bitcoin IBD/sync status.
///
/// Writers may only admit [`Self::Ok`]. [`Self::Unsupported`] and [`Self::Failed`]
/// fail closed before Platform write.
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash, Serialize, Deserialize)]
#[serde(rename_all = "snake_case")]
...
impl_fact_tag!(HoldingSourceStatus, "holding source status", pub, "Returns the canonical snake-case tag used on holding fact responses.", {
    Self::Ok => "ok",
...
impl HoldingSourceStatus {
    ...
    /// Returns whether a report candidate with this status is acceptable in v1.
    pub const fn is_acceptable_for_report(self) -> bool {
        self.is_admissible_for_write()
    }
}
