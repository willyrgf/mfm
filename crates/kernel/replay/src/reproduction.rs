...
use mfm_journal::{FactSelectionCompleteness, JournalHead, PersistedJournalValue, TransitionRef};
use mfm_program::{
...
impl ExactReproductionPlan {
    pub(crate) fn from_verified_export(portable_export: &VerifiedExportStream) -> Result<Self> {
        ...
        let validated =
            RecoverabilityContract::embedded()?.encode(EXACT_REPRODUCTION_PLAN_CONTRACT, &value)?;
        Ok(Self { validated })
    }
...
impl CanonicalReplayResult {
    fn encode(value: &CanonicalValue) -> Result<Self> {
        let validated =
            RecoverabilityContract::embedded()?.encode(REPLAY_RESULT_CONTRACT, value)?;
        Ok(Self { validated })
    }
    ...
    pub fn strict_decode(bytes: &[u8]) -> Result<Self> {
        let validated =
            RecoverabilityContract::embedded()?.strict_decode(REPLAY_RESULT_CONTRACT, bytes)?;
        Ok(Self { validated })
    }
