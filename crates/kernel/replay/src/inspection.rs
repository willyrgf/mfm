...
use mfm_journal::{
    AccessAuditStatus, AuthorizationRef, CapabilityBindingRef, JournalHead, NonDomainFailure,
    ObservationRef, SafeFailure, ValueRef,
};
...
impl CanonicalTransitionTrace {
    pub(crate) fn encode(value: &CanonicalValue) -> Result<Self> {
        let validated =
            RecoverabilityContract::embedded()?.encode(TRANSITION_TRACE_CONTRACT, value)?;
        Ok(Self { validated })
    }
    ...
    pub fn strict_decode(bytes: &[u8]) -> Result<Self> {
        let validated =
            RecoverabilityContract::embedded()?.strict_decode(TRANSITION_TRACE_CONTRACT, bytes)?;
        Ok(Self { validated })
    }
...
impl AccessAuditEntry {
    ...
    /// Returns the exact audit-only non-domain failure, when present.
    pub const fn non_domain_failure(&self) -> Option<&NonDomainFailure> {
        self.non_domain_failure.as_ref()
    }
...
mod tests {
    ...
    fn public_audit_entry_owns_every_reviewed_field_and_terminal_annotation() {
        ...
        assert_eq!(terminal.non_domain_failure(), None);
        ...
        let failed = AccessAuditEntry::new(
            unobserved.authorization_ref().clone(),
            Some(
                ObservationRef::new(&record_ref(&run_id, 4, 'b')).expect("failure observation ref"),
            ),
            unobserved.authorization_journal_head().clone(),
            Some(JournalHead::new(4, &commit_digest('c')).expect("failure head")),
            unobserved.capability_binding_ref().clone(),
            unobserved.capability_operation_id().clone(),
            unobserved.request_ref().clone(),
        ...
        assert_eq!(
            failed.non_domain_failure(),
            Some(&non_domain_failure),
            "the public projection must preserve the exact closed failure tuple"
        );
    }
