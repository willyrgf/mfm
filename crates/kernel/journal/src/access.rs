...
pub use mfm_capabilities::{
    ...
    NonDomainFailureFields, NonDomainFailureLayer,
};
use mfm_ids::{
...
define_schema_value! {
    /// Annex-backed codec for a persisted non-domain entry status.
    pub struct PersistedNonDomainEntryStatus => "mfm.non-domain-entry-status.v1";
    /// Annex-backed codec for a persisted non-domain disposition.
    pub struct PersistedNonDomainDisposition => "mfm.non-domain-disposition.v1";
    /// Annex-backed codec for a persisted non-domain failure code.
    pub struct PersistedNonDomainFailureCode => "mfm.non-domain-failure-code.v1";
    /// Annex-backed codec for a persisted capability-owned non-domain failure.
    pub struct PersistedNonDomainFailure => "mfm.non-domain-failure.v1";
...
impl PersistedNonDomainEntryStatus {
    /// Constructs one closed entry-status value.
    pub fn new(value: NonDomainEntryStatus) -> Result<Self> {
        Self::from_canonical_value(cv_string(value.as_str()))
    }

    /// Projects the closed entry-status value.
    pub fn value(&self) -> Result<NonDomainEntryStatus> {
        match self.canonical_value()? {
            CanonicalValue::String(value) => {
                NonDomainEntryStatus::parse(&value).map_err(Into::into)
            }
...
impl PersistedNonDomainDisposition {
    /// Constructs one closed disposition value.
    pub fn new(value: NonDomainDisposition) -> Result<Self> {
        Self::from_canonical_value(cv_string(value.as_str()))
    }

    /// Projects the closed disposition value.
    pub fn value(&self) -> Result<NonDomainDisposition> {
        match self.canonical_value()? {
            CanonicalValue::String(value) => {
                NonDomainDisposition::parse(&value).map_err(Into::into)
            }
            _ => Err(JournalError::Projection),
        }
    }
...
impl PersistedNonDomainFailureCode {
    /// Constructs one closed non-domain failure-code value.
    pub fn new(value: NonDomainFailureCode) -> Result<Self> {
        Self::from_canonical_value(cv_string(value.as_str()))
    }

    /// Projects the closed failure-code value.
    pub fn value(&self) -> Result<NonDomainFailureCode> {
        match self.canonical_value()? {
            CanonicalValue::String(value) => {
                NonDomainFailureCode::parse(&value).map_err(Into::into)
            }
            _ => Err(JournalError::Projection),
        }
    }
...
impl PersistedNonDomainFailure {
    /// Encodes one capability-owned closed non-domain failure.
    pub fn new(failure: &NonDomainFailure) -> Result<Self> {
        Self::from_canonical_value(
            failure
                .canonical_value()
    ...
    /// Projects and validates the capability-owned closed relation.
    pub fn value(&self) -> Result<NonDomainFailure> {
        NonDomainFailure::from_canonical_value(&self.canonical_value()?).map_err(Into::into)
    }
}

fn cv_non_domain_failure(failure: &NonDomainFailure) -> Result<CanonicalValue> {
    PersistedNonDomainFailure::new(failure)?.canonical_value()
}

fn non_domain_failure_from_canonical(value: CanonicalValue) -> Result<NonDomainFailure> {
    PersistedNonDomainFailure::from_canonical_value(value)?.value()
}
...
pub enum ObservationOutcomeFields {
    ...
    NonDomainFailure {
        /// Closed redaction-safe non-domain failure.
        non_domain_failure: NonDomainFailure,
    },
}
...
impl ObservationOutcome {
    ...
    /// Constructs an audit-only non-domain failure observation.
    pub fn non_domain_failure(non_domain_failure: &NonDomainFailure) -> Result<Self> {
        Self::from_canonical_value(tagged_object(
            "non_domain_failure",
            [(
                "non_domain_failure",
                cv_non_domain_failure(non_domain_failure)?,
            )],
        )?)
    }
    ...
    pub fn fields(&self) -> Result<ObservationOutcomeFields> {
        match super::codec::tag(self)?.as_str() {
            ...
            "non_domain_failure" => Ok(ObservationOutcomeFields::NonDomainFailure {
                non_domain_failure: non_domain_failure_from_canonical(required_field(
                    self,
                    "non_domain_failure",
                )?)?,
            }),
...
pub enum AccessAuditStatus {
    ...
    Indeterminate,
    /// The operation produced an audit-only non-domain failure.
    NonDomainFailure,
}
...
impl AccessAuditStatus {
    ...
    pub const fn as_str(self) -> &'static str {
        match self {
            ...
            Self::NonDomainFailure => "non_domain_failure",
        }
    }
    ...
    fn parse(value: &str) -> Result<Self> {
        match value {
            ...
            "non_domain_failure" => Ok(Self::NonDomainFailure),
            _ => Err(super::JournalError::Projection),
        }
    }
}
...
fn validate_access_audit_failures(
    status: AccessAuditStatus,
    failure: Option<&SafeFailure>,
    non_domain_failure: Option<&NonDomainFailure>,
) -> Result<()> {
    let valid = match status {
        AccessAuditStatus::NonDomainFailure => failure.is_none() && non_domain_failure.is_some(),
        AccessAuditStatus::DidNotEnter | AccessAuditStatus::Indeterminate => {
            failure.is_some() && non_domain_failure.is_none()
        }
        AccessAuditStatus::AuthorizedUnobserved | AccessAuditStatus::Returned => {
            failure.is_none() && non_domain_failure.is_none()
        }
    };
    if valid {
        Ok(())
...
pub struct AccessAuditEntryFields {
    ...
    /// Optional audit-only non-domain failure.
    pub non_domain_failure: Option<NonDomainFailure>,
    /// Optional effect key.
...
impl AccessAuditEntry {
    ...
    pub fn new(
        ...
        validate_access_audit_failures(status, failure, non_domain_failure)?;
        Self::from_canonical_value(object([
            (
                "version",
    ...
    pub fn fields(&self) -> Result<AccessAuditEntryFields> {
        let fields = AccessAuditEntryFields {
            ...
            failure: nullable_field(self, "failure")?
                ...
                .transpose()?,
            non_domain_failure: nullable_field(self, "non_domain_failure")?
                .map(non_domain_failure_from_canonical)
                .transpose()?,
        ...
        validate_access_audit_failures(
            fields.status,
            fields.failure.as_ref(),
            fields.non_domain_failure.as_ref(),
        )?;
        Ok(fields)
    }
    ...
    pub fn validate_unobserved(&self, authorization_ref: &AuthorizationRef) -> Result<()> {
        ...
        if fields.authorization_ref == *authorization_ref
            ...
            && fields.non_domain_failure.is_none()
        {
            Ok(())
        } else {
            Err(super::JournalError::AccessAuditMismatch)
    ...
    pub fn validate_observation(
        ...
        let outcome_matches = match observed.outcome.fields()? {
            ...
            ObservationOutcomeFields::NonDomainFailure { non_domain_failure } => {
                fields.status == AccessAuditStatus::NonDomainFailure
                    && fields.non_domain_failure.as_ref() == Some(&non_domain_failure)
            }
        };
        if outcome_matches {
            Ok(())
