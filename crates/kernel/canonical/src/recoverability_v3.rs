...
static EMBEDDED_CONTRACT: OnceLock<
    std::result::Result<RecoverabilityContractV3, RecoverabilityError>,
> = OnceLock::new();
...
///
/// Construction is private to [`RecoverabilityContractV3`], so callers cannot
/// pair claimed bytes with an unrelated schema identity.
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct ValidatedCanonicalValueV3 {
...
impl ValidatedCanonicalValueV3 {
...
/// [`RecoverabilityContractV3`] so callers cannot pair an arbitrary path with a
/// validated reference value.
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct CanonicalReferencePathV3 {
    pointer: String,
}

impl CanonicalReferencePathV3 {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub enum ReferenceTerminalKindV3 {
...
#[derive(Debug, Clone, PartialEq, Eq)]
pub struct SchemaReferenceEdgeV3 {
    path: CanonicalReferencePathV3,
    declared_contract: String,
    terminal_kind: ReferenceTerminalKindV3,
    value: ValidatedCanonicalValueV3,
}

impl SchemaReferenceEdgeV3 {
    /// Returns the opaque canonical location of the encountered reference.
    pub const fn path(&self) -> &CanonicalReferencePathV3 {
        &self.path
    }
    ...
    pub const fn terminal_kind(&self) -> ReferenceTerminalKindV3 {
        self.terminal_kind
    }

    /// Returns the referenced value revalidated under its resolved terminal schema.
    pub const fn value(&self) -> &ValidatedCanonicalValueV3 {
        &self.value
    }
}
...
#[derive(Debug)]
pub struct RecoverabilityContractV3 {
...
impl RecoverabilityContractV3 {
    ...
    pub fn strict_decode(
        ...
        schema_contract: &str,
        bytes: &[u8],
    ) -> std::result::Result<ValidatedCanonicalValueV3, RecoverabilityError> {
    ...
    pub fn reference_edges(
        &self,
        value: &ValidatedCanonicalValueV3,
    ) -> std::result::Result<Vec<SchemaReferenceEdgeV3>, RecoverabilityError> {
    ...
    fn collect_reference_edges(
        ...
        value: &Value,
        path: &str,
        edges: &mut Vec<SchemaReferenceEdgeV3>,
        depth: usize,
        ...
        match required_string(definition, "kind", RecoverabilityErrorCode::InvalidAnnex)? {
            "reference" => {
                    ...
                    self.resolve_reference_terminal(declared_contract, depth + 1)?
                {
                    ...
                    edges.push(SchemaReferenceEdgeV3 {
                        path: CanonicalReferencePathV3 {
    ...
    fn resolve_reference_terminal(
        ...
        contract: &str,
        depth: usize,
    ) -> std::result::Result<Option<ReferenceTerminalKindV3>, RecoverabilityError> {
        if depth > MAX_SCHEMA_DEPTH {
            return Err(RecoverabilityError::new(
                RecoverabilityErrorCode::InvalidAnnex,
        ...
        match contract {
            "mfm.content-ref.v1" => return Ok(Some(ReferenceTerminalKindV3::ContentRef)),
            "mfm.value-ref.v1" => return Ok(Some(ReferenceTerminalKindV3::ValueRef)),
            _ => {}
        }
...
impl RecoverabilityContractV3 {
    ...
    fn validate_annex_graph(
        ...
        for (contract, descriptor) in schema_descriptors {
            ...
            let (name, version) = contract_name_version(contract).ok_or_else(|| {
                RecoverabilityError::new(
                    RecoverabilityErrorCode::InvalidAnnex,
                    "schema contract has no positive canonical version",
                )
            })?;
            let expected = SchemaId::new(name, version, DigestAlgorithm::Sha256JcsV1, digest)
                .map_err(|_| {
                    RecoverabilityError::new(
                        RecoverabilityErrorCode::InvalidAnnex,
...
fn resolved_union_variants<'a>(
    contract: &'a RecoverabilityContractV3,
    shape: &'a Value,
    depth: usize,
...
fn string_matches_grammar(value: &str, grammar: &str) -> bool {
    match grammar {
        ...
        "mfm.[a-z0-9._-]+.v[1-9][0-9]*" => contract_name_version(value).is_some(),
        "schema:<name>:<positive-version>:sha256-jcs-v1:[0-9a-f]{64}" => SchemaId::parse(value)
            .is_ok_and(|schema| {
                schema.algorithm() == DigestAlgorithm::Sha256JcsV1
                    && schema.version().is_some_and(is_positive_canonical_u64)
            }),
...
fn is_positive_canonical_u64(value: &str) -> bool {
    value != "0" && is_canonical_u64(value)
}

fn contract_name_version(value: &str) -> Option<(&str, &str)> {
    let (name, version) = value.rsplit_once(".v")?;
    if name.len() <= 4
        || !name.starts_with("mfm.")
        || !name[4..].bytes().all(|byte| {
            byte.is_ascii_lowercase() || byte.is_ascii_digit() || matches!(byte, b'.' | b'_' | b'-')
        })
        || !is_positive_canonical_u64(version)
    {
        return None;
    }
    Some((name, version))
...
fn validate_annex_metadata(
    ...
    if !string_matches_grammar(envelope_schema, "mfm.[a-z0-9._-]+.v[1-9][0-9]*") {
        return Err(RecoverabilityError::new(
            RecoverabilityErrorCode::InvalidAnnex,
...
fn validate_logical_key_registry(
    ...
    for entry in required_array(registry, "entries", RecoverabilityErrorCode::InvalidAnnex)? {
        ...
        if !string_matches_grammar(contract, "mfm.[a-z0-9._-]+.v[1-9][0-9]*")
...
fn validate_schema_descriptor(
    ...
    if !string_matches_grammar(contract, "mfm.[a-z0-9._-]+.v[1-9][0-9]*") {
        return Err(RecoverabilityError::new(
            RecoverabilityErrorCode::InvalidAnnex,
            "schema contract violates the versioned contract grammar",
        ));
    }
    validate_predicate_ids(required_array(
        object,
    ...
    let (canonical_name, version) = contract_name_version(contract).ok_or_else(|| {
        RecoverabilityError::new(
            RecoverabilityErrorCode::InvalidAnnex,
            "schema contract has no positive canonical version",
        )
    ...
    if schema_id.algorithm() != DigestAlgorithm::Sha256JcsV1
        || schema_id.version() != Some(version)
        || schema_id.canonical_name() != Some(canonical_name)
    {
        return Err(RecoverabilityError::new(
            RecoverabilityErrorCode::InvalidAnnex,
...
fn validate_domain_descriptor(
    ...
    if !string_matches_grammar(domain, "mfm.[a-z0-9._-]+.v[1-9][0-9]*") {
        return Err(RecoverabilityError::new(
            RecoverabilityErrorCode::InvalidAnnex,
    ...
    if !string_matches_grammar(preimage_schema, "mfm.[a-z0-9._-]+.v[1-9][0-9]*") {
        return Err(RecoverabilityError::new(
            RecoverabilityErrorCode::InvalidAnnex,
            "domain preimage schema violates the versioned contract grammar",
        ));
    }
    let result_kind =
