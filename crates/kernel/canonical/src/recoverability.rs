...
pub struct ValidatedCanonicalValue {
...
/// [`RecoverabilityContract`] so callers cannot pair an arbitrary path with a
/// validated reference value.
#[derive(Debug, Clone, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub struct CanonicalReferencePath {
    pointer: String,
}

impl CanonicalReferencePath {
...
#[derive(Debug, Clone, Copy, PartialEq, Eq, PartialOrd, Ord, Hash)]
pub enum ReferenceTerminalKind {
...
impl SchemaReferenceEdge {
    ...
    pub const fn terminal_kind(&self) -> ReferenceTerminalKind {
        self.terminal_kind
    }

    /// Returns the referenced value revalidated under its resolved terminal schema.
    pub const fn value(&self) -> &ValidatedCanonicalValue {
        &self.value
    }
}
...
impl RecoverabilityContract {
    ...
    pub fn strict_decode(
        ...
        schema_contract: &str,
        bytes: &[u8],
    ) -> std::result::Result<ValidatedCanonicalValue, RecoverabilityError> {
    ...
    pub fn reference_edges(
        &self,
        value: &ValidatedCanonicalValue,
    ) -> std::result::Result<Vec<SchemaReferenceEdge>, RecoverabilityError> {
    ...
    fn collect_reference_edges(
        ...
        value: &Value,
        path: &str,
        edges: &mut Vec<SchemaReferenceEdge>,
        depth: usize,
        ...
        match required_string(definition, "kind", RecoverabilityErrorCode::InvalidAnnex)? {
            "reference" => {
                    ...
                    self.resolve_reference_terminal(declared_contract, depth + 1)?
                {
                    ...
                    edges.push(SchemaReferenceEdge {
                        path: CanonicalReferencePath {
    ...
    fn resolve_reference_terminal(
        ...
        contract: &str,
        depth: usize,
    ) -> std::result::Result<Option<ReferenceTerminalKind>, RecoverabilityError> {
        if depth > MAX_SCHEMA_DEPTH {
            return Err(RecoverabilityError::new(
                RecoverabilityErrorCode::InvalidAnnex,
        ...
        match contract {
            "mfm.content-ref.v1" => return Ok(Some(ReferenceTerminalKind::ContentRef)),
            "mfm.value-ref.v1" => return Ok(Some(ReferenceTerminalKind::ValueRef)),
            _ => {}
        }
...
fn resolved_union_variants<'a>(
    contract: &'a RecoverabilityContract,
    shape: &'a Value,
    depth: usize,
