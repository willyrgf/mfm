...
use mfm_spec::CertifiedStateExecution;
    ...
    pub(super) fn prepare_existing_append(
        ...
        match material {
            ...
            ExistingRunAppendMaterial::Observation(material) => {
                prepare_observation(self, append_request_id, *material, false)
                    .map(PreparedJournalAppend::from)
            }
        }
    }

    pub(super) fn prepare_observation_resolution(
        &self,
        append_request_id: AppendRequestId,
        material: ObservationMaterial,
    ) -> Result<PreparedJournalAppend> {
        prepare_observation(self, append_request_id, material, true)
            .map(PreparedJournalAppend::from)
    }
}
...
fn prepare_observation(
    ...
    match material {
        ObservationMaterial::Read {
            authorization_ref,
            outcome,
        } => prepare_read_observation(
            view,
            append_request_id,
            &authorization_ref,
            *outcome,
            allow_already_observed,
        ),
        ...
        ObservationMaterial::FactSelection { sealed } => {
            let (observation, objects, pending) =
                prepare_fact_selection_observation(view, *sealed)?;
            ObserveExternalAccess::new(
                view,
                append_request_id,
                observation,
                objects,
        ...
        ObservationMaterial::FactSelectionFailure {
            ...
            append_request_id,
            &authorization_ref,
            ReadObservationMaterial::NonDomainFailure { failure },
            allow_already_observed,
        ),
    }
}
...
fn prepare_read_observation(
    ...
    let entry = access_entry(view, authorization_ref, allow_already_observed)?;
    ...
    let (outcome, authorities) = match material {
        ...
        ReadObservationMaterial::NonDomainFailure { failure } => {
            let layer = if authorization.capability_operation_id.as_str()
                == super::FACT_SELECTION_OPERATION_ID
            {
                mfm_journal::NonDomainFailureLayer::Fact
            } else {
                mfm_journal::NonDomainFailureLayer::Read
            };
            failure
                .validate_layer(layer)
                .map_err(|_| StoreError::AuthorizationNotEligible)?;
            (
                ObservationOutcome::non_domain_failure(&failure)?,
                Vec::new(),
            )
        }
    };
...
fn prepare_safe_failure(
    ...
    frozen_contract: &mfm_spec::RetainedValueContract,
    indeterminate: bool,
...
fn prepare_effect_observation(
    ...
    access_entry(view, authorization_ref, allow_already_observed)?;
    match outcome.into_parts() {
        mfm_executor::EffectExecutorOutcomeParts::Returned(result) => {
            prepare_returned_effect_observation(
                view,
                append_request_id,
                authorization_ref,
                result,
        ...
        mfm_executor::EffectExecutorOutcomeParts::DidNotEnter(failure) => {
            prepare_effect_safe_failure_observation(
                view,
                append_request_id,
                authorization_ref,
                failure,
        ...
        mfm_executor::EffectExecutorOutcomeParts::Indeterminate(failure) => {
            prepare_effect_safe_failure_observation(
                view,
                append_request_id,
                authorization_ref,
        ...
        mfm_executor::EffectExecutorOutcomeParts::NonDomainFailure(failure) => {
            failure
                .validate_layer(mfm_journal::NonDomainFailureLayer::Ensure)
                .map_err(|_| StoreError::AuthorizationNotEligible)?;
            let outcome = ObservationOutcome::non_domain_failure(&failure)?;
            let observation = ExternalAccessObserved::new(authorization_ref, &outcome, None)?;
            let objects = PreparedObjectGraph::prepare_for_record_values(
                &[observation.canonical_value()?],
                Vec::new(),
...
fn access_entry<'a>(
    ...
    if entry.observation().is_some() && !allow_already_observed {
