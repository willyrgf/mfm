...
use mfm_canonical::{
    ...
    ReferenceTerminalKind, ValidatedCanonicalValue,
};
...
use mfm_spec::CanonicalExpansionPath;
...
impl VerifiedTransitionTrace {
    pub(super) fn new(canonical_value: CanonicalValue) -> Result<Self> {
        RecoverabilityContract::embedded()?.encode(TRANSITION_TRACE_CONTRACT, &canonical_value)?;
...
fn read_evidence(
    ...
    let (mut values, permits_scan_attestation) = match observation.outcome.fields()? {
        ...
        ObservationOutcomeFields::NonDomainFailure { .. } => (Vec::new(), false),
    };
...
fn terminal_effect_evidence(
    ...
    while let Some(value_ref) = pending.pop_front() {
        ...
        for edge in contract.reference_edges(&validated)? {
            if edge.terminal_kind() == ReferenceTerminalKind::ValueRef {
...
fn settlement_trace(
    ...
    blocking_sources: &[mfm_journal::BlockingSource],
) -> Result<(
    CanonicalValue,
    Vec<CanonicalValue>,
    Vec<CanonicalValue>,
...
fn retained_value_from_validated(
    fields: &mfm_journal::ValueRefFields,
...
mod tests {
    ...
    use crate::{StoreAuthorityContext, StoreError, StoreIdentity};
