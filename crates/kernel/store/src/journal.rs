...
use mfm_canonical::{CanonicalValue, PlainCanonicalJsonBytes, RecoverabilityContract};
...
use mfm_journal::{
    ...
    NonDomainDisposition, NonDomainFailure, NonDomainFailureLayer, ObjectPathBinding,
    ObservationOutcome, ObservationOutcomeFields, ObservationRef, OutputBinding, OutputRef,
...
pub struct FoldedAccessAuditEntry {
    ...
    non_domain_failure: Option<NonDomainFailure>,
...
impl<'page> VerifiedAccessAuditEntry<'page> {
    ...
    /// Returns the exact audit-only non-domain failure when one was observed.
    pub const fn non_domain_failure(&self) -> Option<&'page NonDomainFailure> {
        match self.entry {
            VerifiedAccessAuditSource::Page(entry) => entry.non_domain_failure.as_ref(),
            VerifiedAccessAuditSource::Folded(entry) => entry.non_domain_failure.as_ref(),
        }
    }
...
pub enum PendingEffectStatus {
    ...
    Indeterminate,
    /// The latest ensure ended in a retryable audit-only non-domain failure.
    NonDomainRetryable,
    /// The latest ensure ended in an integrity-blocking non-domain failure.
    NonDomainIntegrityBlocked,
}

fn pending_effect_status(
    status: AccessAuditStatus,
    non_domain_failure: Option<&NonDomainFailure>,
) -> Result<PendingEffectStatus> {
    Ok(match status {
        ...
        AccessAuditStatus::NonDomainFailure => {
            match non_domain_failure
                .ok_or(StoreError::PersistedMismatch {
                    field: "non_domain_failure",
                })?
                .fields()
                .disposition
            {
                NonDomainDisposition::RetryableOperational => {
                    PendingEffectStatus::NonDomainRetryable
                }
                NonDomainDisposition::IntegrityBlocked => {
                    PendingEffectStatus::NonDomainIntegrityBlocked
                }
            }
        }
    })
...
pub struct FoldedPendingEffect {
    ...
    executor_binding_ref: mfm_journal::CapabilityBindingRef,
...
impl FoldedPendingEffect {
    ...
    pub const fn executor_binding_ref(&self) -> &mfm_journal::CapabilityBindingRef {
...
struct TransitionApplication<'a> {
    ...
    transition: StateTransitionCommitted,
    closure: Option<mfm_journal::RunClosed>,
...
impl FoldState {
    ...
    fn apply_observation(&mut self, application: ObservationApplication<'_>) -> Result<()> {
        ...
        let (status, failure, non_domain_failure, returned_ref) =
            audit_outcome_fields(&observation)?;
        ...
        authorization.entry.non_domain_failure = non_domain_failure;
...
fn verify_fact_selection_observation(
    ...
    node: &CertifiedNodeContract,
    authorization_ref: &AuthorizationRef,
    authorization: &mfm_journal::ExternalAccessAuthorizedFields,
    observation: &mfm_journal::ExternalAccessObservedFields,
) -> Result<()> {
    ...
    let result_ref = match observation.outcome.fields()? {
        ObservationOutcomeFields::Returned { result_ref } => result_ref,
        ObservationOutcomeFields::NonDomainFailure { non_domain_failure } => {
            non_domain_failure
                .validate_layer(NonDomainFailureLayer::Fact)
                .map_err(|_| StoreError::FactScanBindingMismatch)?;
...
fn verify_read_observation(
    ...
    validate_this_record_producer(frozen_ref, FROZEN_READ_INTENT_PATH)?;
    let frozen = mfm_journal::FrozenReadIntent::strict_decode(&resolver.value_bytes(frozen_ref)?)?;
    let frozen = frozen.fields()?;
    if frozen.node_id != *node.node_id()
        || frozen.input_manifest_ref != *input_manifest_ref
        || frozen.state_contract_ref != *node.state_contract_ref()
    ...
    match &outcome {
        ...
        ObservationOutcomeFields::NonDomainFailure { non_domain_failure } => {
            non_domain_failure
                .validate_layer(NonDomainFailureLayer::Read)
                .map_err(|_| observation_mismatch("read_non_domain_failure"))?;
            verify_exact_observation_values(resolver, authorization_ref, &[], false)?;
        }
    }
    Ok(())
    ...
    match &outcome {
        ...
        ObservationOutcomeFields::NonDomainFailure { non_domain_failure } => {
            non_domain_failure
                ...
                .map_err(|_| observation_mismatch("effect_non_domain_failure"))?;
            verify_exact_observation_values(resolver, authorization_ref, &[], false)
        }
    }
}
    ...
    match selector {
        ...
        CertifiedSourceSelector::Seed { source_field_path } => {
            ...
            let entries = manifest
                ...
                .collect::<std::result::Result<Vec<_>, mfm_journal::JournalError>>()?;
...
fn resolve_observation_cross_run(
    resolver: &ObservationObjectResolver<'_>,
    admission: &mfm_journal::RunAdmittedFields,
...
type AuditOutcomeFields = (
    AccessAuditStatus,
    Option<SafeFailure>,
    Option<NonDomainFailure>,
    Option<ValueRef>,
);

fn audit_outcome_fields(observation: &ExternalAccessObserved) -> Result<AuditOutcomeFields> {
    Ok(match observation.fields()?.outcome.fields()? {
        ObservationOutcomeFields::Returned { result_ref } => {
            (AccessAuditStatus::Returned, None, None, Some(result_ref))
        }
        ObservationOutcomeFields::DidNotEnter { safe_failure } => (
            AccessAuditStatus::DidNotEnter,
            Some(safe_failure),
            None,
            None,
        ),
...
fn same_retained_value_shape(
    left: &mfm_journal::ValueRefFields,
    right: &mfm_journal::ValueRefFields,
) -> bool {
...
impl VerifiedRunView {
    ...
    pub fn pending_effects(&self) -> Result<Vec<FoldedPendingEffect>> {
        self.certified_spec
            ...
            .map(|(node_id, pending)| {
                ...
                let mut status = PendingEffectStatus::NotAuthorized;
                for state in self.fold.authorizations.iter().rev() {
                    let fields = state.entry.authorization.fields()?;
                    match fields.scope.fields()? {
                        AuthorizationScopeFields::EnsureEffect {
                            effect_request_transition_ref,
                        } if effect_request_transition_ref == request_transition_ref => {
                            let audit = state.entry.audit_entry.fields()?;
                            status = pending_effect_status(
                                audit.status,
                                audit.non_domain_failure.as_ref(),
                            )?;
                            break;
                        }
                        AuthorizationScopeFields::Read { .. }
        ...
        for state in &self.fold.authorizations {
            ...
            entries.push(AccessAuditProjection {
                ...
                failure: observation_visible
                    ...
                    .flatten(),
                non_domain_failure: observation_visible
                    .then_some(state.entry.non_domain_failure)
                    .flatten(),
    ...
    pub(super) fn validate_prepared_transition(
        ...
        self.validate_sequence_reservation(0)?;
    ...
    pub(super) fn validate_prepared_authorization(
        ...
        self.validate_candidate_predecessor(candidate)?;
        self.validate_sequence_reservation(1)?;
        let fields = candidate.fields()?;
        validate_object_envelope(
            &fields.ordered_object_bindings,
            &fields.artifact_admission_intents,
    ...
    fn validate_sequence_reservation(&self, unmatched_delta: i8) -> Result<()> {
        let current_sequence = self.journal_head().fields()?.run_sequence;
        let unmatched = self
            .fold
            .authorizations
            .iter()
            .filter(|entry| entry.entry.observation.is_none())
            .count();
        let unmatched = u64::try_from(unmatched).map_err(|_| StoreError::SequenceOverflow)?;
        validate_successor_sequence_capacity(current_sequence, unmatched, unmatched_delta)
    }
}

pub(super) fn validate_successor_sequence_capacity(
    current_sequence: u64,
    unmatched_authorizations: u64,
    unmatched_delta: i8,
) -> Result<()> {
    let successor_sequence = current_sequence
        .checked_add(1)
        .ok_or(StoreError::SequenceOverflow)?;
    let unmatched_after = match unmatched_delta {
        -1 => unmatched_authorizations
            .checked_sub(1)
            .ok_or(StoreError::UnknownAuthorization)?,
        0 => unmatched_authorizations,
        1 => unmatched_authorizations
            .checked_add(1)
            .ok_or(StoreError::SequenceOverflow)?,
        _ => {
            return Err(StoreError::InvalidPreparedAppend {
                purpose: "sequence_reservation",
                message: "invalid unmatched-authorization delta",
            });
        }
    };
    successor_sequence
        .checked_add(unmatched_after)
        .ok_or(StoreError::SequenceOverflow)
        .map(drop)
}
...
pub(super) fn committed_candidate_preimage(
    commit: &CommittedJournalCommit,
) -> Result<CommitCandidatePreimage> {
    let fields = commit.envelope().fields()?;
    let candidates = commit
        .records()
        .iter()
        .map(|record| record.candidate().clone())
        .collect::<Vec<_>>();
    CommitCandidatePreimage::new(
        &fields.core.predecessor,
        batch_purpose(&normalize_candidates(&candidates)?)?,
        &candidates,
        &fields.core.ordered_object_bindings,
        &fields.core.artifact_admission_intents,
    )
    .map_err(Into::into)
}
    ...
    for (index, commit) in commits.iter().enumerate() {
        ...
        match (&prior_head, envelope.core.predecessor.fields()?) {
            (
                ...
                let derived = mfm_journal::GenesisPreimage::new(
...
fn verify_batch_coordinate(
    ...
    coordinate: &mfm_journal::TenantFactCoordinate,
) -> Result<()> {
...
fn verify_root_manifest(
    ...
    prefix: &str,
    entries: Vec<mfm_journal::RootManifestEntry>,
