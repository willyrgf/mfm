...
use super::compiler::{
    request_digest, ArtifactIntent, FactClaimArtifact, OperationOutcomeArtifact,
    StateOutcomeArtifact,
};
...
impl ReducedRunState {
    ...
    pub(super) fn admission(&self) -> Result<&RunAdmitted, StructuredStoreError> {
        self.working.admission.as_ref().ok_or_else(invalid)
    }
...
#[derive(Debug, Clone, PartialEq, Eq)]
// Record payloads stay inline until the compiler consumes the reducer result.
#[allow(clippy::large_enum_variant)]
pub(super) enum RecordIntent {
...
impl UnboundSuccessor {
    pub(super) fn bind(
        mut self,
        assignments: &[RecordRef],
        journal_head: JournalHead,
    ) -> Result<ReducedRunState, StructuredStoreError> {
        bind_working_state(&mut self.working, assignments)?;
        let cursor = bind_cursor(self.cursor, assignments)?;
...
pub(super) struct PendingSemanticStep {
    pub(super) successor: Box<UnboundSuccessor>,
    pub(super) records: Vec<RecordIntent>,
    pub(super) artifacts: Vec<ArtifactIntent>,
    pub(super) tenant_fact_requirement: TenantFactRequirement,
    pub(super) semantic_facts: Vec<CommittedFactRef>,
    pub(super) obligations: Vec<SemanticObligation>,
}
...
impl PendingSemanticStep {
    pub(super) fn semantic_eq(&self, other: &Self) -> bool {
        self.successor == other.successor
            ...
            && self.semantic_facts == other.semantic_facts
...
fn finish_step(
    ...
    artifacts: Vec<ArtifactIntent>,
    tenant_fact_requirement: TenantFactRequirement,
    semantic_facts: Vec<CommittedFactRef>,
...
fn reduce_admission(
    ...
    if previous.working.admission.is_some() || previous.run_id() != &context.admission.run_id {
        return Err(invalid());
    }
    ...
    let admission = context.admission.record(digest.clone());
    if context
        .admission
        .recorded_genesis
        .as_ref()
    ...
    working.admission = Some(admission.clone());
    working.admission_handle = Some(RecordHandle::Current(0));
    working.semantic_head = Some(WorkingSemanticHead::Genesis {
        admission: RecordHandle::Current(0),
        digest,
    });
    let mut records = vec![RecordIntent::Admission(admission)];
    let expected_closure = match &cursor {
        WorkCursor::Completed { outcome_ref } => Some(RunClosed {
            outcome_ref: outcome_ref.clone(),
        }),
        _ => None,
    };
    if let Some(closure) = expected_closure {
        working.closed_outcome_ref = Some(closure.outcome_ref.clone());
        records.push(RecordIntent::Closure(closure));
    }
    artifacts.extend(
        context
            ...
            .iter()
            .cloned()
            .map(ArtifactIntent::QualifiedObject),
    );
    finish_step(
        working,
        cursor,
...
fn reduce_transition(
    ...
    let (outcome, facts) = match value {
        ...
        TransitionValueIntent::Failure(value) => {
            ...
            (StateOutcomeRef::Failure(binding), Vec::new())
        }
    };
...
fn reduce_transition_parts(
    ...
    mut artifacts: Vec<ArtifactIntent>,
) -> Result<PendingSemanticStep, StructuredStoreError> {
    ...
    let outcome_artifact = match &outcome {
        StateOutcomeRef::Success(value) => StateOutcomeArtifact::Success(value.clone()),
        StateOutcomeRef::Failure(value) => StateOutcomeArtifact::Failure(value.clone()),
    };
    let outcome_ref = outcome_artifact.content_ref().map_err(|_| invalid())?;
    artifacts.push(ArtifactIntent::StateOutcome(outcome_artifact));
    ...
    working.semantic_head = Some(WorkingSemanticHead::Transition {
        transition: RecordHandle::Current(0),
    ...
    let mut records = vec![RecordIntent::Transition(transition.clone())];
    if let Some(closure) = expected_closure {
        working.closed_outcome_ref = Some(closure.outcome_ref.clone());
        records.push(RecordIntent::Closure(closure));
    }
...
fn reduce_authorization_intent(
    ...
    let record = author_authorization(
        context,
        previous,
        &action,
        state,
    ...
    artifacts.push(ArtifactIntent::QualifiedObject(
        physical_binding_ref.clone(),
    ));
    reduce_authorization_parts(context, previous, record, artifacts)
}
...
fn reduce_recorded_authorization(
    ...
    reduce_authorization_parts(context, previous, authorization.clone(), artifacts)
}
...
fn reduce_authorization_parts(
    ...
    let action = previous.action()?.clone();
    let state = occurrence(context, &action.occurrence_id)?;
    validate_authorization(context, previous, &action, state, &record)?;
    let capability = context.live_component(&record.capability_contract_ref)?;
    ...
    let previous_physical_binding_ref = if minimum_lineage_head_ref.is_some() {
        previous
            ...
            .map(|authorization| authorization.record.physical_binding_ref.clone())
    } else {
        None
    };
    let obligation = if is_fact_selection {
        SemanticObligation::PriorRunSelection {
            ...
            certificate_ref: record.physical_binding_ref.clone(),
        }
    } else {
        SemanticObligation::PhysicalAuthorization {
            access_kind: record.access_kind,
            capability_contract_ref: record.capability_contract_ref.clone(),
            capability_implementation_ref: record.capability_implementation_ref.clone(),
            adapter_contract_ref: record.adapter_contract_ref.clone(),
            adapter_implementation_ref: record.adapter_implementation_ref.clone(),
            admitted_routing_policy_ref: record.admitted_routing_policy_ref.clone(),
    ...
    if working
        .authorizations
        .contains_key(&record.access_attempt_id)
    {
        return Err(invalid());
    }
    working
    ...
    finish_step(
        ...
        cursor,
        bindings,
        vec![RecordIntent::Authorization(record)],
        artifacts,
...
fn reduce_observation_intent(
    ...
    let authorization = previous
        ...
        .ok_or_else(invalid)?;
    let capability = context.live_component(&authorization.record.capability_contract_ref)?;
    let protocol = capability
        .capability_protocol
        .as_ref()
        .ok_or_else(invalid)?;
    ...
    let outcome = match outcome {
        ObservationValueIntent::Returned(value) => ObservationOutcome::Returned {
            value: typed_value(
                context,
        ...
        ObservationValueIntent::SafeFailure(value) => ObservationOutcome::SafeFailure {
            value: typed_value(
                context,
        ...
        ObservationValueIntent::EntryUnknown { fault_code } => ObservationOutcome::EntryUnknown {
...
fn reduce_recorded_observation(
    ...
    reduce_observation_parts(context, previous, observation.clone(), artifacts)
}
...
fn reduce_observation_parts(
    ...
    if matches!(observation.outcome, ObservationOutcome::EntryUnknown { .. })
        && authorization.record.access_kind != AccessKind::Effect
    {
        return Err(invalid());
    }
    ...
    } = &observation.outcome
    {
        ...
        obligations.push(SemanticObligation::PhysicalSupersession {
            capability_contract_ref: authorization.record.capability_contract_ref.clone(),
            adapter_contract_ref: authorization.record.adapter_contract_ref.clone(),
            adapter_implementation_ref: authorization.record.adapter_implementation_ref.clone(),
            authorized_binding_ref: authorization.record.physical_binding_ref.clone(),
    ...
    finish_step(
        ...
        cursor,
        bindings,
        vec![RecordIntent::Observation(observation)],
        artifacts,
        TenantFactRequirement::None,
        Vec::new(),
        obligations,
...
fn require_open(previous: &ReducedRunState) -> Result<(), StructuredStoreError> {
    if previous.working.admission.is_none() || previous.working.closed_outcome_ref.is_some() {
        return Err(invalid());
    }
    Ok(())
}
...
fn validate_settleable(
    ...
    match (&action.execution_kind, &action.leaf) {
        ...
        (
            ...
            if actual_ref != observation_ref || authorization.record.access_kind != expected_kind {
                return Err(invalid());
            }
            Ok(())
        }
...
fn author_authorization(
    ...
    let mut record = ExternalAccessAuthorized {
...
fn validate_authorization(
    ...
    let expected = author_authorization(
        ...
        action,
        state,
        &record.state_input_ref,
        record.request.clone(),
        record.request_digest.clone(),
        record.physical_binding_ref.clone(),
    )?;
    if &expected != record {
        return Err(invalid());
    }
    if matches!(action.leaf, StateLeaf::Reassertable { .. }) {
        let predecessor = previous
            ...
            .and_then(|attempt| previous.working.authorizations.get(attempt))
            .ok_or_else(invalid)?;
        if predecessor.record.occurrence_id != record.occurrence_id
            || predecessor.record.state_input_ref != record.state_input_ref
            || predecessor.record.request.contract_ref != record.request.contract_ref
            || predecessor.record.request_digest != record.request_digest
        {
            return Err(invalid());
        }
    }
    if previous
        ...
        && !matches!(
            (record.access_kind, &action.leaf),
...
fn typed_value(
    ...
    value: CanonicalJsonValue,
    artifacts: &mut Vec<ArtifactIntent>,
    ...
    let value_ref = intent.content_ref()?;
...
fn require_typed_value(
    ...
    if artifact.content_ref()? != value.value_ref {
        return Err(invalid());
    }
...
fn prepare_fact_intents(
    ...
    facts: &FactSet,
    artifacts: &mut Vec<ArtifactIntent>,
) -> Result<Vec<CommittedFactRef>, StructuredStoreError> {
    ...
    for slot in &state.contract.fact_slots {
            ...
            .is_some_and(|proposal| proposal.fact_slot_ordinal() == slot.fact_slot_ordinal())
        {
            ...
            let claim = FactClaimArtifact {
                descriptor_ref: proposal.descriptor_ref().clone(),
                subject: subject.clone(),
                response: response.clone(),
            };
            let claim_ref = claim.content_ref().map_err(|_| invalid())?;
            artifacts.push(ArtifactIntent::FactClaim(claim));
...
fn validate_recorded_facts(
    ...
    for slot in &state.contract.fact_slots {
            ...
            .is_some_and(|fact| fact.fact_slot_ordinal == slot.fact_slot_ordinal())
        {
            ...
            if claim.content_ref().map_err(|_| invalid())? != fact.claim_ref {
                return Err(invalid());
            }
            artifacts.push(ArtifactIntent::FactClaim(claim));
...
impl<'a> Engine<'a> {
    ...
    fn from_previous(
        ...
        for artifact in &artifacts {
            if let ArtifactIntent::TypedValue { value, .. } = artifact {
                generated.insert(artifact.content_ref()?, value.clone());
            }
        }
        Ok(Self {
            context,
    ...
    fn derive_root(&mut self) -> Result<WorkCursor, StructuredStoreError> {
        match walk_block(self, &self.context.program.expanded().root)? {
            ...
            WalkResult::Complete(flow) => {
                let (artifact, value) = match flow {
                    FlowValue::Normal(value) => (
                        OperationOutcomeArtifact::Success(value.reference.clone()),
                        value,
                    ),
                    FlowValue::ScopeFailure(value) => (
                        OperationOutcomeArtifact::Failure(value.reference.clone()),
                        value,
                    ),
                };
                let _ = value;
                let outcome_ref = artifact.content_ref().map_err(|_| invalid())?;
                self.artifacts
                    .push(ArtifactIntent::OperationOutcome(artifact));
    ...
    fn bind_derived_with_origin(
        ...
        let value_ref = artifact.content_ref()?;
...
fn walk_state(
    ...
    if let Some(transition) = engine.working.transitions.get(&state.occurrence_id) {
        return walk_committed_state(engine, state, input, &transition.record);
    }
...
fn state_leaf(
    ...
    let Some(observation) = working.observations.get(attempt_id) else {
        return match authorization.record.access_kind {
...
fn bind_handle(
    ...
    match handle {
        ...
        RecordHandle::Current(ordinal) => assignments
...
fn bind_working_state(
    ...
    if let Some(handle) = &mut working.admission_handle {
        *handle = RecordHandle::Existing(bind_handle(handle, assignments)?);
    }
    for transition in working.transitions.values_mut() {
        transition.handle = RecordHandle::Existing(bind_handle(&transition.handle, assignments)?);
    }
    for authorization in working.authorizations.values_mut() {
        authorization.handle =
            RecordHandle::Existing(bind_handle(&authorization.handle, assignments)?);
    }
    for observation in working.observations.values_mut() {
        observation.handle = RecordHandle::Existing(bind_handle(&observation.handle, assignments)?);
    }
    if let Some(head) = &mut working.semantic_head {
        match head {
            WorkingSemanticHead::Genesis { admission, .. } => {
                *admission = RecordHandle::Existing(bind_handle(admission, assignments)?);
            }
            WorkingSemanticHead::Transition { transition, .. } => {
                *transition = RecordHandle::Existing(bind_handle(transition, assignments)?);
            }
        }
    }
    Ok(())
...
fn bind_semantic_head(
    ...
    match head {
        WorkingSemanticHead::Genesis { admission, digest } => Ok(SemanticHead::Genesis {
            admission_ref: bind_handle(admission, assignments)?,
            semantic_state_digest: digest.clone(),
        }),
        WorkingSemanticHead::Transition { transition, digest } => Ok(SemanticHead::Transition {
            transition_ref: bind_handle(transition, assignments)?,
            semantic_state_digest: digest.clone(),
        }),
    }
}

fn bind_leaf(leaf: WorkLeaf, assignments: &[RecordRef]) -> Result<StateLeaf, StructuredStoreError> {
    Ok(match leaf {
        ...
        WorkLeaf::ObservedForSettlement {
            ...
            access_attempt_id,
            observation_ref: bind_handle(&observation, assignments)?,
        },
...
fn bind_action(
    ...
    Ok(ActionableState {
        ...
        leaf: bind_leaf(action.leaf, assignments)?,
    })
}
...
fn bind_lane(
    ...
    Ok(match lane {
        WorkCursor::AtState(action) => LaneCursor::AtState(bind_action(action, assignments)?),
        WorkCursor::InFanOut { group_path, lanes } => LaneCursor::InFanOut {
            group_path,
            lanes: lanes
                .into_iter()
                .map(|lane| bind_lane(lane, assignments))
                .collect::<Result<Vec<_>, _>>()?,
        },
...
fn bind_cursor(
    ...
    Ok(match cursor {
        WorkCursor::AtState(action) => ProgramCursor::AtState(bind_action(action, assignments)?),
