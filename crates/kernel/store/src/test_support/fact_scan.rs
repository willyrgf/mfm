...
pub(crate) struct FactSelectionFixtureContracts {
    pub(crate) execution: FixtureReadExecution,
    pub(crate) routing_generation_ref: ContentRef,
    pub(crate) request_contract: RetainedValueContract,
    #[cfg(test)]
    pub(crate) response_contract: RetainedValueContract,
    #[cfg(test)]
    pub(crate) attestation_contract: RetainedValueContract,
    pub(crate) request: FactSelectionRequest,
}

pub(crate) fn fact_selection_contracts(
    discriminator: u8,
...
async fn admit_fixture<B>(
    ...
    support: &crate::AdmittedSupportGraph,
) -> std::result::Result<RunId, <B as RunJournalBackend>::Error>
where
    B: crate::FactScanBackend + ConfiguredValueBackend + AdmissionSourceBackend,
{
    ...
    B: crate::FactScanBackend,
{
    ...
    let output_ordinals = closed
        ...
        .collect::<mfm_journal::Result<Vec<_>>>()
        .map_err(StoreError::from)
        .map_err(<B as RunJournalBackend>::Error::from)?;
