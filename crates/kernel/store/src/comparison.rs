...
pub struct VerifiedComparisonFrame<'view> {
    ...
    containing_journal_head: mfm_journal::JournalHead,
...
impl<'view> VerifiedComparisonFrame<'view> {
    ...
    pub const fn containing_journal_head(&self) -> &mfm_journal::JournalHead {
...
fn safe_failure_value(
    ...
    safe_failure: mfm_journal::SafeFailure,
    contract: &RetainedValueContract,
...
fn comparison_facts(
    view: &VerifiedRunView,
    node: &CertifiedNodeContract,
    emissions: Vec<mfm_journal::FactEmission>,
    ...
    let facts = emissions
        ...
        .map(|emission| {
            ...
            verify_fact_producer(
                ...
                mfm_journal::FactValueComponent::Subject,
            )?;
            verify_fact_producer(
                view,
                node,
                &claim.response_ref,
                fields.emission_ordinal,
                mfm_journal::FactValueComponent::Response,
            )?;
...
fn verify_fact_producer(
    ...
    value_ref: &ValueRef,
    emission_ordinal: u32,
    component: mfm_journal::FactValueComponent,
) -> Result<()> {
    match value_ref.fields()?.producer_binding.fields()? {
