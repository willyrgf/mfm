...
use mfm_journal::v1::{
    ...
    GenesisPreimage, JournalHead, JournalPredecessor, LegalCommitBatch, RecordHashPreimage,
    RecordIdPreimage, RecordLogicalKey, RecordRef, RunAdmitted, RunClosed, RunJournalRecord,
    RunPhase, StateTransitionCommitted, TenantFactCoordinate, TenantFactCoordinateFields,
    TenantFactFrontier, TransitionBodyFields, TransitionSlot,
};
...
impl ObserveExternalAccess {
    ...
    pub(super) fn new(
        ...
        let returned = matches!(
            ...
            mfm_journal::v1::ObservationOutcomeFields::Returned { .. }
        );
...
/// The only four append variants accepted by a recoverability-v2 run journal.
...
impl AssignedJournalAppend {
    fn new(
        ...
        let expected_sequence = match core.expected_predecessor.fields()? {
            mfm_journal::v1::JournalPredecessorFields::Genesis { .. } => 1,
            mfm_journal::v1::JournalPredecessorFields::JournalHead(head) => head
                .run_sequence
                .checked_add(1)
