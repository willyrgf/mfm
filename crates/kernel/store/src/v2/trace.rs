...
use mfm_canonical::{
    ...
    ReferenceTerminalKindV3, ValidatedCanonicalValueV3,
};
...
fn terminal_effect_evidence(
    ...
    while let Some(value_ref) = pending.pop_front() {
        ...
        for edge in contract.reference_edges(&validated)? {
            if edge.terminal_kind() == ReferenceTerminalKindV3::ValueRef {
...
fn settlement_trace(
    ...
    blocking_sources: &[mfm_journal::v2::BlockingSource],
) -> Result<(
    CanonicalValue,
    Vec<CanonicalValue>,
    Vec<CanonicalValue>,
...
fn retained_value_from_validated(
    fields: &mfm_journal::v2::ValueRefFields,
...
mod tests {
    ...
    use crate::v2::{StoreAuthorityContext, StoreError, StoreIdentity};
