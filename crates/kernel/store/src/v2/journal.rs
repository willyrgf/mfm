...
use mfm_canonical::{CanonicalValue, PlainCanonicalJsonBytes, RecoverabilityContractV3};
...
pub struct FoldedPendingEffect {
    ...
    executor_binding_ref: mfm_journal::v2::CapabilityBindingRef,
...
impl FoldedPendingEffect {
    ...
    pub const fn executor_binding_ref(&self) -> &mfm_journal::v2::CapabilityBindingRef {
...
struct TransitionApplication<'a> {
    ...
    transition: StateTransitionCommitted,
    closure: Option<mfm_journal::v2::RunClosed>,
...
fn verify_fact_selection_observation(
    ...
    node: &CertifiedNodeContract,
    authorization_ref: &AuthorizationRef,
    authorization: &mfm_journal::v2::ExternalAccessAuthorizedFields,
    observation: &mfm_journal::v2::ExternalAccessObservedFields,
) -> Result<()> {
...
fn verify_read_observation(
    ...
    validate_this_record_producer(frozen_ref, FROZEN_READ_INTENT_PATH)?;
    let frozen =
        mfm_journal::v2::FrozenReadIntent::strict_decode(&resolver.value_bytes(frozen_ref)?)?;
    let frozen = frozen.fields()?;
    if frozen.node_id != *node.node_id()
        || frozen.input_manifest_ref != *input_manifest_ref
        || frozen.state_contract_ref != *node.state_contract_ref()
...
fn resolve_observation_cross_run(
    resolver: &ObservationObjectResolver<'_>,
    admission: &mfm_journal::v2::RunAdmittedFields,
...
fn same_retained_value_shape(
    left: &mfm_journal::v2::ValueRefFields,
    right: &mfm_journal::v2::ValueRefFields,
) -> bool {
    ...
    for (index, commit) in commits.iter().enumerate() {
        ...
        match (&prior_head, envelope.core.predecessor.fields()?) {
            (
                ...
                let derived = mfm_journal::v2::GenesisPreimage::new(
...
fn verify_batch_coordinate(
    ...
    coordinate: &mfm_journal::v2::TenantFactCoordinate,
) -> Result<()> {
...
fn verify_root_manifest(
    ...
    prefix: &str,
    entries: Vec<mfm_journal::v2::RootManifestEntry>,
