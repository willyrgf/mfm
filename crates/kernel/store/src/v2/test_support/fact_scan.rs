...
use crate::v2::{
    ...
    CommittedJournalCommit, CompletedFactScan, ConfiguredValueBackend, Drive,
    ExistingRunAppendMaterial, FactAttestationLoadVerifier, FactSelectionAuthorizationOutcome,
    NewlyAppended, ObjectGraphProposal, ObservationMaterial, PreparedJournalAppend,
    ProducedObjectRoot, ProducedOutputSlot, QualifiedRunStore, QualifiedSupportMember, Result,
    RunAccessAuthority, RunAccessAuthorityIssuer, RunHistoryReader, RunHistoryWriter,
    RunJournalBackend, SettlementMaterial, StoreError, StoreIdentity, SupportBackend,
    TransitionMaterial, VerifiedRunView, FACT_SELECTION_OPERATION_ID,
};

pub(in crate::v2) struct FactSelectionFixtureContracts {
    pub(in crate::v2) execution: FixtureReadExecution,
    pub(in crate::v2) routing_generation_ref: ContentRef,
    pub(in crate::v2) request_contract: RetainedValueContract,
    #[cfg(test)]
    pub(in crate::v2) response_contract: RetainedValueContract,
    #[cfg(test)]
    pub(in crate::v2) attestation_contract: RetainedValueContract,
    pub(in crate::v2) request: FactSelectionRequest,
}

pub(in crate::v2) fn fact_selection_contracts(
    discriminator: u8,
...
impl FactScanConformanceFixture {
    ...
    pub fn new(
        ...
        let primitive_schema = RecoverabilityContractV3::embedded()?
            .schema_id("mfm.primitive-canonical_value.v1")?
            .clone();
    ...
    /// Produces one genuine completed affine scan for observation-retry tests.
    ///
    /// The caller must provision the producer and consumer configured values.
    /// This helper stops before sealing or preparing the observation so the
    /// runtime test can prove that the completed scan crosses that boundary
    /// exactly once.
    pub async fn completed_scan_on<B>(
        &self,
        store: QualifiedRunStore<B>,
        issuer: &RunAccessAuthorityIssuer,
    ) -> std::result::Result<CompletedFactScan, <B as RunJournalBackend>::Error>
    where
        B: crate::v2::FactScanBackend
            + SupportBackend
            ...
            + AdmissionSourceBackend,
    {
        ...
        .await?;
        let authorized = authorize_fact_selection(
            &writer,
            issuer,
            self,
            &consumer_run_id,
            "fact-runtime-retry-completed",
        )
        .await?;
        let completed =
            scan_fact_selection_with_fact_limit(writer.backend(), authorized.permit, 2).await?;
...
struct AuthorizedFactScan {
    permit: crate::v2::FactScanPermit,
    authorization_ref: mfm_journal::v2::AuthorizationRef,
...
async fn admit_fixture<B>(
    ...
    support: &crate::v2::AdmittedSupportGraph,
) -> std::result::Result<RunId, <B as RunJournalBackend>::Error>
where
    B: crate::v2::FactScanBackend + ConfiguredValueBackend + AdmissionSourceBackend,
{
...
fn verify_committed_fact_observation_modes(commit: &CommittedJournalCommit) -> Result<()> {
    ...
    let bound = envelope
        ...
        .collect::<mfm_journal::v2::Result<BTreeSet<_>>>()?;
    let mut products = 0;
    let mut imports = 0;
