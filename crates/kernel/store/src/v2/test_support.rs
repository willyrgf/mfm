...
fn content_ref(contract: &RetainedValueContract, bytes: &[u8]) -> Result<ContentRef> {
    ContentRef::new(
        ...
        RecoverabilityContractV3::embedded()?.raw_content_digest(bytes),
    )
    .map_err(Into::into)
}
...
#[cfg(test)]
mod tests {
    use mfm_canonical::{CanonicalJsonBytes, PlainCanonicalJsonBytes, RecoverabilityContractV3};
    ...
    async fn legal_fixture_prepares_appends_and_projects_a_pure_settlement() {
        ...
        assert_eq!(cross_tenant_error, crate::v2::StoreError::RunNotFound);
    ...
    async fn trace_requirements(
        ...
        root: &crate::v2::RunAccessAuthority<crate::v2::InspectTrace>,
    ) -> crate::v2::TransitionTraceSourceRequirements {
        store
            .discover_transition_trace_sources(
                root,
