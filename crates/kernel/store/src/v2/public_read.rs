...
use mfm_journal::v2::{ProducerBindingFields, RunPhase};
...
impl VerifiedPublicRunView {
    pub(super) fn project(view: &VerifiedRunView) -> Result<Self> {
        ...
        let validated =
            RecoverabilityContractV3::embedded()?.encode(PUBLIC_RUN_VIEW_CONTRACT, &value)?;
        Ok(Self { validated })
    }

    /// Consumes this sealed projection into its exact annex-validated canonical response.
    pub fn into_validated(self) -> ValidatedCanonicalValueV3 {
        self.validated
    }
}
...
const fn pending_effect_status(status: PendingEffectStatus) -> &'static str {
    match status {
        ...
        PendingEffectStatus::NonDomainRetryable => "non_domain_retryable",
        PendingEffectStatus::NonDomainIntegrityBlocked => "non_domain_integrity_blocked",
    }
}
