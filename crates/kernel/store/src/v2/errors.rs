...
/// Error returned by the recoverability-v3 store contract.
...
impl From<mfm_journal::v2::JournalError> for StoreError {
    fn from(_: mfm_journal::v2::JournalError) -> Self {
...
/// Backend-proven provenance of a failure returned after a fact-scan permit is consumed.
#[derive(Debug, Clone, Copy, PartialEq, Eq)]
pub enum FactScanFailureProvenance {
    /// The backend could not complete its reviewed operational IO protocol.
    StoreUnavailable,
    /// Retained or deployment authority state failed an integrity requirement.
    HistoryInvalid,
    /// The caller/backend adapter violated the already-authorized scan protocol.
...
pub trait StoreErrorInspection {
    ...
    /// Classifies a failure returned after consuming a fact-scan permit.
...
impl StoreErrorInspection for StoreError {
    ...
    fn fact_scan_failure_provenance(&self) -> FactScanFailureProvenance {
        use FactScanFailureProvenance::{
            AdapterContractViolation, HistoryInvalid, StoreUnavailable,
        };

        match self {
            Self::MemoryLockPoisoned | Self::InjectedFailure { .. } => StoreUnavailable,
            Self::JournalContract
            | Self::EmptyJournal
            | Self::PersistedMismatch { .. }
            | Self::SequenceOverflow
            | Self::FactOrderOverflow { .. }
            | Self::InvalidObjectAuthority { .. }
            | Self::MissingObjectAuthority { .. }
            | Self::ObjectContentMismatch { .. }
            | Self::ObjectAuthorityConflict { .. }
            | Self::ObjectNotReachable
            | Self::InvalidFactCoordinate
            | Self::CorruptFactHistory
            | Self::SourceScopeMismatch
            | Self::InvalidSourceClosure
            | Self::DigestMismatch { .. } => HistoryInvalid,
            Self::RunNotFound
            | Self::AccessDenied { .. }
            | Self::InvalidAuthorityBinding { .. }
            | Self::AdmissionAuthorityMismatch
            | Self::AdmissionConflict
            | Self::InvalidPreparedAppend { .. }
            | Self::HeadMismatch { .. }
            | Self::AppendRunNotFound { .. }
            | Self::AppendRequestConflict
            | Self::RunClosed
            | Self::InvalidClosure
            | Self::DuplicateLogicalRecord
            | Self::AuthorizationNotEligible
            | Self::UnknownAuthorization
            | Self::ObservationAlreadyCommitted
            | Self::InvalidAuditTail
            | Self::InvalidAuditPage { .. }
            | Self::InvalidTracePage { .. }
            | Self::FrozenReadIntentConflict
            | Self::ObservationNotConsumable
            | Self::TransitionFoldMismatch { .. }
            | Self::FactSelectionLimitExceeded
            | Self::FactScanBindingMismatch
            | Self::MemoryFailureSelectorAlreadyArmed => AdapterContractViolation,
        }
    }
}
...
#[cfg(test)]
mod tests {
    use super::{FactScanFailureProvenance, StoreError, StoreErrorInspection};

    #[test]
    fn fact_scan_failure_provenance_is_typed_and_closed() {
        assert_eq!(
            StoreError::InjectedFailure { point: "scan" }.fact_scan_failure_provenance(),
            FactScanFailureProvenance::StoreUnavailable
        );
        assert_eq!(
            StoreError::CorruptFactHistory.fact_scan_failure_provenance(),
            FactScanFailureProvenance::HistoryInvalid
        );
        assert_eq!(
            StoreError::FactScanBindingMismatch.fact_scan_failure_provenance(),
