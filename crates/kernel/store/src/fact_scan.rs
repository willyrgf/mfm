...
use mfm_canonical::{RecoverabilityContract, ReferenceTerminalKind};
...
use super::{
    ...
    CommittedObject, Drive, NewlyAppended, ObjectAuthorityKey, PreparedJournalAppend,
    PreparedObjectGraph, Replay, Result, RunAccessAuthority, RunHistoryReader, RunHistoryWriter,
    RunJournalBackend, StoreError, StoreIdentity, VerifiedRunView, FACT_SELECTION_OPERATION_ID,
};
...
/// Immutable completed fact-selection result retained while its observation is pending.
///
/// Construction consumes the affine scan result, so retrying observation persistence cannot
/// recreate or rerun the authorized scan.
#[derive(Clone)]
pub struct SealedFactSelectionObservation {
    store_identity: StoreIdentity,
    tenant_scope_id: TenantScopeId,
    consuming_run_id: RunId,
    authorization_ref: AuthorizationRef,
...
impl CompletedFactScan {
    ...
    /// Consumes this affine result into immutable pending observation material.
    ///
    /// The scan permit cannot survive this boundary. The resulting material may be cloned only to
    /// retry persistence of the same logical response.
    pub fn seal(self) -> SealedFactSelectionObservation {
        SealedFactSelectionObservation {
            store_identity: self.permit.store_identity,
            tenant_scope_id: self.permit.tenant_scope_id,
            consuming_run_id: self.permit.consuming_run_id,
            authorization_ref: self.permit.authorization_ref,
            request: self.permit.request,
            frontier: self.permit.frontier,
...
struct PrefixClosureWalker<'a> {
    contract: &'static RecoverabilityContract,
    ...
    fn payload_reference_frames(
        ...
        self.contract
            ...
            .map(|edge| match edge.terminal_kind() {
                ...
                ReferenceTerminalKind::ValueRef => ValueRef::strict_decode(edge.value().as_bytes())
                    .map(ClosureVisitFrame::EnterValue)
                    .map_err(Into::into),
            })
            .collect()
...
pub(super) fn prepare_fact_selection_observation(
    view: &VerifiedRunView,
    completed: SealedFactSelectionObservation,
) -> Result<(
    ExternalAccessObserved,
    PreparedObjectGraph,
    PendingFactScanAttestation,
)> {
    if &completed.store_identity != view.store_identity()
        || &completed.tenant_scope_id != view.tenant_scope_id()
        || &completed.consuming_run_id != view.run_id()
    {
        return Err(StoreError::FactScanBindingMismatch);
    }
    let entry = view
    ...
    let qualified =
        qualify_fact_scan(view, &completed.authorization_ref, Some(&completed.request))?;
...
fn validate_spec_value_contract(
    contract: &mfm_spec::RetainedValueContract,
    value_ref: &ValueRef,
) -> Result<()> {
        ...
        Box::pin(async move {
            ...
            for transition in view.transition_entries() {
                let fields = transition
                    ...
                    .map_err(StoreError::from)
                    .map_err(B::Error::from)?;
                let mfm_journal::TransitionBodyFields::ReadSettled {
