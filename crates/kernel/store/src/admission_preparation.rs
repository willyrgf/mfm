...
pub(super) fn prepare_admission_material(
    ...
    push_artifact(
        &mut authorities,
        "authored_program",
        &mfm_spec::CanonicalAuthoredProgram::retained_contract()?,
    ...
    push_artifact(
        &mut authorities,
        "certificate",
        &mfm_spec::Certificate::retained_contract()?,
...
fn validate_input(
    ...
    RecoverabilityContract::embedded()?.strict_decode_schema_id(
            ...
            binding.ordered_sources().iter().map(|source| {
                (
                    ...
                    match binding.destination() {
                        mfm_spec::CertifiedInputDestination::EvidenceOnly {
                            certified_evidence_role_ref,
                        } => Some(certified_evidence_role_ref),
                        mfm_spec::CertifiedInputDestination::OrdinaryValue => None,
                    },
                )
            })
        }))
