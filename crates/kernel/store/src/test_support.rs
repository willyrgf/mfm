...
impl LegalAdmissionFixture {
    ...
    fn certification_closure(&self) -> Result<FixtureCertificationClosure> {
        ...
        let expanded_spec = mfm_spec::ExpandedCertifiedSpec::new(
...
fn content_ref(contract: &RetainedValueContract, bytes: &[u8]) -> Result<ContentRef> {
    ContentRef::new(
        ...
        RecoverabilityContract::embedded()?.raw_content_digest(bytes),
    )
    .map_err(Into::into)
}
...
#[cfg(test)]
mod tests {
    use mfm_canonical::{CanonicalJsonBytes, PlainCanonicalJsonBytes, RecoverabilityContract};
    ...
    async fn legal_fixture_prepares_appends_and_projects_a_pure_settlement() {
        ...
        assert_eq!(cross_tenant_error, crate::StoreError::RunNotFound);
    ...
    async fn trace_requirements(
        ...
        root: &crate::RunAccessAuthority<crate::InspectTrace>,
    ) -> crate::TransitionTraceSourceRequirements {
        store
            .discover_transition_trace_sources(
                root,
