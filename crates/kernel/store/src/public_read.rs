...
use mfm_journal::{ProducerBindingFields, RunPhase};
...
impl VerifiedPublicRunView {
    pub(super) fn project(view: &VerifiedRunView) -> Result<Self> {
        ...
        let validated =
            RecoverabilityContract::embedded()?.encode(PUBLIC_RUN_VIEW_CONTRACT, &value)?;
        Ok(Self { validated })
    }

    /// Consumes this sealed projection into its exact annex-validated canonical response.
    pub fn into_validated(self) -> ValidatedCanonicalValue {
        self.validated
    }
}
