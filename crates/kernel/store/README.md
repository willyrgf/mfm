...
Production run mutation is `append_prepared_commit_plan(PreparedCommitPlan)`. Plans are built from
purpose-specific `PreparedCommit<Purpose>` authority and contain the event payload batch plus
artifact evidence that must become run authority atomically with that batch. Store implementations
own sequence, ordinal, event id, logical-key, precondition, artifact evidence, and projection
validation.
