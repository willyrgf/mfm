...
fn identical_run_id_preimages_derive_identical_ids() {
    ...
    let invocation = InvocationIdentity::new("00000000-0000-4000-8000-000000000001").expect("inv");
    let first = derive_run_id(&identity.store_scope_id, &tenant, &operation, &invocation);
    let second = derive_run_id(&identity.store_scope_id, &tenant, &operation, &invocation);
...
fn each_run_id_preimage_field_changes_identity() {
    ...
    let operation = StableId::new("mfm.test/entry").expect("op");
    let invocation = InvocationIdentity::new("00000000-0000-4000-8000-000000000001").expect("inv");
    let base = derive_run_id(&identity.store_scope_id, &tenant, &operation, &invocation);
    let other_scope =
        StoreScopeId::new(format!("{}{}", StoreScopeId::PREFIX, "3".repeat(32))).expect("scope");
    assert_ne!(
        base,
        derive_run_id(&other_scope, &tenant, &operation, &invocation)
    );
    let other_tenant =
        TenantScopeId::new(format!("{}{}", TenantScopeId::PREFIX, "4".repeat(32))).expect("tenant");
    assert_ne!(
        base,
        derive_run_id(
    ...
    assert_ne!(
        base,
        derive_run_id(&identity.store_scope_id, &tenant, &other_op, &invocation)
    );
    let other_inv = InvocationIdentity::new("00000000-0000-4000-8000-000000000099").expect("inv");
    assert_ne!(
        base,
        derive_run_id(&identity.store_scope_id, &tenant, &operation, &other_inv)
    );
}
